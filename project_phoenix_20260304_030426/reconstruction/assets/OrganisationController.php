...
class OrganisationController extends Controller
{
    ...
    public function store(StoreOrganisationRequest $request): RedirectResponse
    {
        ...
        error_log('🔵 OrganisationController::store() called at ' . date('Y-m-d H:i:s'));
        ...
        try {
            ...
            try {
                Mail::to($organisation->email)->send(
                    new OrganisationCreatedMail($organisation, $user)
                );
            } catch (\Exception $e) {
            ...
            // For Inertia 2.0: Return redirect response
            // Inertia automatically handles the redirect on the frontend
            return redirect()
            ...
            // For Inertia 2.0: Return back with error message
            return back()
                ->with('error', 'Fehler beim Erstellen der Organisation: ' . $e->getMessage())
                ->withInput();
        }
    }
