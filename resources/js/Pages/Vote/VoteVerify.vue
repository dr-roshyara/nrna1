        ...
        <div class="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-8">
            <div class="container mx-auto px-4 max-w-7xl">
                ...
                <div class="text-center mb-10">
                    ...
                    <!-- Progress Indicator -->
                    <div class="w-full max-w-md mx-auto bg-neutral-200 rounded-full h-2 mb-2">
                        <div class="bg-gradient-to-r from-green-500 to-emerald-500 h-2 rounded-full"
                             style="width: 80%"></div>
                    </div>
                    <p class="text-sm text-neutral-500">{{ $t('pages.vote_verify.header.progress_complete') }}</p>
                </div>
                ...
                <div class="grid lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="bg-gradient-to-br from-red-50 to-orange-50 border-2 border-danger-200 rounded-2xl p-6 shadow-lg">
                            <div class="flex items-start">
                                ...
                                <div class="ml-4">
                                    <h3 class="text-xl font-bold text-danger-800 mb-2">
                                        ⚠️ {{ $t('pages.vote_verify.critical_alert.title') }}
                                    </h3>
                                    <p class="text-danger-700 mb-3 font-medium">
                                        {{ $t('pages.vote_verify.critical_alert.description') }}
                                    </p>
                                    <p class="text-danger-600 text-sm">
                                        <strong class="text-danger-800">{{ $t('pages.vote_verify.critical_alert.important_label') }}</strong> महत्वपूर्ण: प्रमाणीकरण कोड बिना तपाईंका छनौटहरू हराउनेछन्।
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-2xl shadow-xl p-8">
                            <div class="mb-8">
                                <h2 class="text-2xl font-bold text-neutral-900 mb-3 flex items-center">
                            ...
                            <!-- Email Notification -->
                            <div class="mb-6 p-5 bg-gradient-to-r from-blue-50 to-indigo-50 border border-primary-200 rounded-xl">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <p class="text-primary-800 font-medium">{{ $t('pages.vote_verify.verification_section.email_notification') }}</p>
                                        <p class="text-primary-600 text-sm">{{ $t('pages.vote_verify.verification_section.email_sent') }}</p>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="mt-4 flex justify-center space-x-2">
                                            <div v-for="i in 8" :key="i"
                                                ...
                                                <span v-else class="text-neutral-400 text-lg">_</span>
                                            </div>
                                        </div>
                        ...
                        <div class="bg-white rounded-2xl shadow-md p-6">
                            <h3 class="text-lg font-bold text-neutral-900 mb-4">{{ $t('pages.vote_verify.help_section.title') }}</h3>
                            <div class="grid md:grid-cols-2 gap-4">
                                <div class="p-4 bg-primary-50 rounded-lg">
                                    <h4 class="font-medium text-primary-800 mb-2">{{ $t('pages.vote_verify.help_section.no_code_title') }}</h4>
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white rounded-2xl shadow-xl p-6 sticky top-8">
                            <!-- User Info Card -->
                            <div class="mb-6 p-5 bg-gradient-to-br from-gray-50 to-blue-50 border border-neutral-200 rounded-xl">
                                <div class="flex items-center mb-3">
                                    ...
                                    <div class="ml-3">
                                        <h3 class="font-bold text-neutral-900">{{ $t('pages.vote_verify.vote_summary.voter_info') }}</h3>
                                        <p class="text-xs text-neutral-500">{{ $t('pages.vote_verify.vote_summary.voter_details') }}</p>
                                    </div>
                                </div>
                                <div class="space-y-2 text-sm">
                                    <div class="flex justify-between">
                                        <span class="text-neutral-600">{{ $t('pages.vote_verify.vote_summary.name_label') }}</span>
                                        <span class="font-medium text-neutral-900">{{ user_info.name }}</span>
                                    </div>
                                    <div v-if="user_info.user_id" class="flex justify-between">
                                        <span class="text-neutral-600">{{ $t('pages.vote_verify.vote_summary.user_id_label') }}</span>
                                        <span class="font-medium text-neutral-900">{{ user_info.user_id }}</span>
                                    </div>
                                    <div v-if="user_info.region" class="flex justify-between">
                                        <span class="text-neutral-600">{{ $t('pages.vote_verify.vote_summary.region_label') }}</span>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="grid grid-cols-3 gap-3 mb-6">
                                    ...
                                    <div class="bg-gradient-to-br from-gray-100 to-gray-200 p-3 rounded-lg text-center">
                                        <div class="text-2xl font-bold text-neutral-600">{{ voting_summary.no_vote_posts || 0 }}</div>
                                        <div class="text-xs text-neutral-700 font-medium">{{ $t('pages.vote_verify.vote_summary.stats_skipped') }}</div>
                                    </div>
                                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 p-3 rounded-lg text-center">
                                        <div class="text-2xl font-bold text-primary-600">{{ voting_summary.total_posts || 0 }}</div>
                                        <div class="text-xs text-primary-700 font-medium">{{ $t('pages.vote_verify.vote_summary.stats_total') }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div v-if="vote_data.national_posts && vote_data.national_posts.length > 0">
                                        <h4 class="font-semibold text-neutral-700 mb-2 text-sm uppercase tracking-wider">{{ $t('pages.vote_verify.vote_summary.national_posts') }}</h4>
                                        <div class="space-y-2">
                                            <div v-for="post in vote_data.national_posts" :key="`national-${post.post_id}`"
                                                ...
                                                <div class="flex justify-between items-start mb-1">
                                                    <span class="text-sm font-medium text-neutral-800">{{ post.post_name }}</span>
                                    ...
                                    <div v-if="vote_data.regional_posts && vote_data.regional_posts.length > 0">
                                        <h4 class="font-semibold text-neutral-700 mb-2 text-sm uppercase tracking-wider">{{ $t('pages.vote_verify.vote_summary.regional_posts') }}</h4>
                                        <div class="space-y-2">
                                    ...
                                    <div v-if="!vote_data.national_posts?.length && !vote_data.regional_posts?.length"
                                        ...
                                        <p class="text-neutral-500 text-sm">{{ $t('pages.vote_verify.vote_summary.no_selections') }}</p>
                                    </div>
                                </div>
                            </div>

                            <!-- Security Badge -->
                            <div class="mt-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-primary-200 rounded-xl">
                                <div class="flex items-center">
                                    <svg class="w-5 h-5 text-primary-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div>
                                        <p class="font-medium text-primary-800">{{ $t('pages.vote_verify.vote_summary.security_badge') }}</p>
                                        <p class="text-primary-600 text-xs">{{ $t('pages.vote_verify.vote_summary.security_description') }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
