...
export default {
    ...
    props: {
        ...
        default_election_type: {
            type: String,
            default: 'real',
            validator: (value) => ['real', 'demo'].includes(value)
        }
    },
    ...
    setup(props) {
        ...
        const form = useForm({
            voting_code: "",
            demo_voting_code: props.verification_code || "",
            electionType: props.default_election_type,
        });

        function submit() {
            const isDemo = form.electionType === 'real';
            const code = isDemo ? form.voting_code : form.demo_voting_code;

            console.log("🔍 Submitting code:", code, "for election type:", form.electionType);
            ...
            const submitForm = useForm({
                voting_code: code,
                election_type: form.electionType
            });

            // ✅ RICHTIG: Dynamischer Route-Name basierend auf electionType
            const routeName = isDemo
                ? 'vote.submit_code_to_view_vote'
                : 'demo.vote.submit_code_to_view_vote';

            console.log("📡 Submitting to route:", routeName);

            submitForm.post(route(routeName), {
                preserveScroll: true,
                onStart: () => {
    ...
    mounted() {
        // Focus input based on election type
        this.$nextTick(() => {
            const inputId = this.form.electionType === 'demo' 
                ? 'voting_code_demo' 
                : 'voting_code_real';
