    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          ...
          <div class="text-3xl font-bold text-primary-600 mt-2">{{ quickStats?.completed ? 1 : 0 }}</div>
        </div>
      </div>
          ...
          <div class="space-y-4">
            ...
            >
                ...
                <span
                  :class="
                    election.can_vote
                      ...
                      : 'px-3 py-1 bg-neutral-100 text-neutral-800 rounded-full text-sm font-medium'
                  "
                >
                  {{
                    election.can_vote
              ...
              <div class="flex gap-2">
                ...
                <button class="flex-1 px-4 py-2 bg-neutral-600 text-white rounded-md hover:bg-neutral-700 transition-colors">
        ...
        <div v-if="votingHistory.length > 0" class="overflow-x-auto">
          <table class="w-full">
            <thead class="bg-neutral-50 border-b">
              <tr>
                <th class="px-6 py-3 text-left text-sm font-medium text-neutral-700">
            ...
            <tbody>
              <tr v-for="vote in votingHistory" :key="vote.id" class="border-b hover:bg-neutral-50">
                <td class="px-6 py-4 text-sm text-neutral-900">{{ vote.date }}</td>
                <td class="px-6 py-4 text-sm text-neutral-900">{{ vote.election }}</td>
                <td class="px-6 py-4 text-sm text-neutral-900">{{ vote.type }}</td>
              </tr>
            </tbody>
          </table>
        </div>
