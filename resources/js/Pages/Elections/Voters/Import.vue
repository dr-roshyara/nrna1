      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
          ...
          <div class="flex items-start justify-between gap-4">
            <div>
              <h1 class="text-3xl font-bold text-neutral-900 mb-1">{{ t.title }}</h1>
              <p class="text-neutral-500 text-sm">
                <span class="font-medium text-neutral-700">{{ election.name }}</span>
                · {{ organisation.name }}
              </p>
              <p class="text-neutral-600 mt-1">{{ t.description }}</p>
            </div>
            <Link
              :href="route('elections.voters.import.tutorial', { organisation: organisation.slug, election: election.slug })"
              class="flex-shrink-0 inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-primary-700 bg-primary-50 hover:bg-primary-100 border border-primary-200 rounded-lg transition-colors"
            >
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div class="lg:col-span-3">
            ...
            <div v-if="currentStep === 'done'"
              ...
              <p v-if="$page.props.flash?.success" class="text-neutral-600 mb-6">
            ...
            <section v-if="currentStep === 'preview' && previewData" class="bg-white rounded-lg shadow-xs p-6 mb-6">
              ...
              <div v-else class="flex gap-4 mb-6">
                <div class="flex-1 bg-primary-50 border border-primary-200 rounded-lg p-3 text-center">
                  <div class="text-2xl font-bold text-primary-700">{{ previewData.stats.new }}</div>
                  <div class="text-xs text-primary-600">New Users</div>
                </div>
              ...
              <div v-if="uses_full_membership" class="bg-neutral-50 rounded-lg border border-neutral-200 overflow-hidden mb-6">
                  ...
                  <table class="w-full text-sm">
                    ...
                    <tbody>
                      <tr v-for="row in previewData.preview" :key="row.row"
                        ...
                        <td class="px-3 py-2">
                          <span :class="row.errors?.length ? 'text-danger-700 font-medium' : 'text-green-700 font-medium'">
                            {{ row.status }}
                          </span>
                        </td>
                ...
                <div class="overflow-x-auto max-h-[32rem] overflow-y-auto">
                  <table class="w-full text-sm">
                    <thead class="sticky top-0 bg-neutral-100 border-b border-neutral-200">
                      <tr>
                        <th class="px-3 py-2 text-left font-medium text-neutral-700">{{ t.preview.table.row }}</th>
                        <th class="px-3 py-2 text-left font-medium text-neutral-700">Name</th>
                        <th class="px-3 py-2 text-left font-medium text-neutral-700">{{ t.preview.table.email }}</th>
                        <th class="px-3 py-2 text-left font-medium text-neutral-700">{{ t.preview.table.status }}</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="row in previewData.preview" :key="row.row"
                          :class="['border-b border-neutral-200', row.status === 'invalid' ? 'bg-danger-50' : 'hover:bg-neutral-100']">
                        <td class="px-3 py-2 text-neutral-500">{{ row.row }}</td>
                        <td class="px-3 py-2 text-neutral-900">
                          <span v-if="row.firstname || row.lastname">{{ [row.firstname, row.lastname].filter(Boolean).join(' ') }}</span>
                          <span v-else class="text-neutral-500 italic">—</span>
                        </td>
                        <td class="px-3 py-2 text-neutral-900">{{ row.email }}</td>
                        <td class="px-3 py-2">
                          <span v-if="row.status === 'new'" class="inline-flex items-center gap-1 px-2 py-1 bg-primary-100 text-primary-700 rounded text-xs font-medium">
                          ...
                          <span v-else class="text-danger-700 font-medium">Invalid</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
              ...
              <div v-else>
                ...
                <div v-if="previewData.stats.new > 0 || previewData.stats.existing > 0" class="p-3 bg-primary-50 border border-primary-200 rounded-lg mb-4 text-sm text-primary-800">
