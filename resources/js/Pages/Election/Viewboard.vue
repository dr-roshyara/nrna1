        ...
        <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 py-8">
            <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
                ...
                <header class="text-center mb-12">
                    ...
                    <p class="text-xl text-neutral-600 mb-4">Election Viewboard — {{ election.name }}</p>
                    <div class="w-24 h-1 bg-primary-600 mx-auto rounded-full"></div>
                    <p class="text-sm text-neutral-500 mt-3 italic">Read-only view | पठन-मात्र दृश्य</p>
                </header>

                <!-- Current Election Status -->
                <section class="mb-12">
                    <div class="bg-white rounded-2xl shadow-lg p-8 border border-neutral-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div class="text-center p-6 rounded-xl" :class="election.is_active ? 'bg-green-50 border border-green-200' : 'bg-danger-50 border border-danger-200'">
                                <div class="flex items-center justify-center mb-4">
                                    <div class="rounded-full p-3" :class="election.is_active ? 'bg-green-100' : 'bg-danger-100'">
                                        <svg class="w-8 h-8" :class="election.is_active ? 'text-green-600' : 'text-danger-600'" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <h3 class="text-lg font-semibold mb-2" :class="election.is_active ? 'text-green-800' : 'text-danger-800'">
                                    चुनाव प्रणाली | Election System
                                </h3>
                                <p class="text-sm" :class="election.is_active ? 'text-green-700' : 'text-danger-700'">
                                    {{ election.is_active ? 'सक्रिय | Active' : 'निष्क्रिय | Inactive' }}
                                </p>
                            </div>

                            <!-- Voting Period Status -->
                            <div class="text-center p-6 rounded-xl" :class="isVotingActive ? 'bg-yellow-50 border border-yellow-200' : 'bg-neutral-50 border border-neutral-200'">
                                <div class="flex items-center justify-center mb-4">
                                    <div class="rounded-full p-3" :class="isVotingActive ? 'bg-yellow-100' : 'bg-neutral-100'">
                                        <svg class="w-8 h-8" :class="isVotingActive ? 'text-yellow-600' : 'text-neutral-600'" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z"/>
                                        </svg>
                                    </div>
                                </div>
                                <h3 class="text-lg font-semibold mb-2" :class="isVotingActive ? 'text-yellow-800' : 'text-neutral-800'">
                                    मतदान अवधि | Voting Period
                                </h3>
                                <p class="text-sm" :class="isVotingActive ? 'text-yellow-700' : 'text-neutral-700'">
                                    {{ isVotingActive ? 'सक्रिय | Active' : 'निष्क्रिय | Inactive' }}
                                </p>
                            </div>

                            <!-- Results Publication Status -->
                            <div class="text-center p-6 rounded-xl" :class="election.results_published ? 'bg-primary-50 border border-primary-200' : 'bg-neutral-50 border border-neutral-200'">
                                <div class="flex items-center justify-center mb-4">
                                    <div class="rounded-full p-3" :class="election.results_published ? 'bg-primary-100' : 'bg-neutral-100'">
                                        <svg class="w-8 h-8" :class="election.results_published ? 'text-primary-600' : 'text-neutral-600'" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M16,11V3H8v6H2v12h20V11H16z M10,5h4v14h-4V5z M4,11h4v8H4V11z M20,19h-4v-6h4V19z"/>
                                        </svg>
                                    </div>
                                </div>
                                <h3 class="text-lg font-semibold mb-2" :class="election.results_published ? 'text-primary-800' : 'text-neutral-800'">
                                    चुनाव परिणाम | Election Results
                                </h3>
                                <p class="text-sm" :class="election.results_published ? 'text-primary-700' : 'text-neutral-700'">
                ...
                <section class="mb-12" v-if="stats && Object.keys(stats).length">
                    <div class="bg-white rounded-2xl shadow-lg p-8 border border-neutral-100">
                        <h2 class="text-2xl font-semibold text-neutral-900 mb-8 text-center">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            ...
                            <!-- Eligible Voters -->
                            <div class="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl border border-primary-200">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-sm font-medium text-primary-600">Eligible Voters</p>
                                        <p class="text-2xl font-bold text-primary-800">{{ stats.eligible_voters ?? 0 }}</p>
                                        <p class="text-xs text-primary-600">Not expired</p>
                                    </div>
                        ...
                        <!-- Status breakdown -->
                        <div class="bg-neutral-50 p-6 rounded-xl" v-if="stats.by_status">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <div class="text-center">
                                    ...
                                    <p class="text-xs text-neutral-600">Invited</p>
                                </div>
                                <div class="text-center">
                                    <p class="text-2xl font-bold text-neutral-500">{{ stats.by_status.inactive ?? 0 }}</p>
                                    <p class="text-xs text-neutral-600">Inactive</p>
                                </div>
                                <div class="text-center">
                                    <p class="text-2xl font-bold text-danger-500">{{ stats.by_status.removed ?? 0 }}</p>
                                    <p class="text-xs text-neutral-600">Removed</p>
                                </div>
                            </div>
                        </div>
                    </div>
