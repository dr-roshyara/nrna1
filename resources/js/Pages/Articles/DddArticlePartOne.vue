  ...
  <div class="min-h-screen bg-gradient-to-b from-neutral-100 to-neutral-50 flex flex-col">
    ...
    <main class="flex-1 relative">
      ...
      <article class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32">
        ...
        <!-- Article content -->
        <div class="space-y-6" v-html="processedContent"></div>
...
const processedContent = computed(() => {
  ...
  let html = props.article.content
    ...
    .replace(/^### (.*?)$/gm, '<h3 class="text-2xl font-serif font-bold mt-8 mb-4 text-neutral-900">$1</h3>')
    .replace(/^## (.*?)$/gm, '<h2 class="text-3xl font-serif font-bold mt-10 mb-5 text-neutral-900">$1</h2>')
    .replace(/^# (.*?)$/gm, '<h1 class="text-4xl font-serif font-bold mt-12 mb-6 text-neutral-900">$1</h1>')
    // Convert markdown links to HTML
    .replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" class="text-primary-600 hover:text-primary-700 underline transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 rounded">$1</a>')
    ...
    .map(para => {
      ...
      if (para.trim() === '') return ''
      return `<p class="text-neutral-700 text-lg leading-relaxed font-light">${para.trim()}</p>`
    })
    .join('')
