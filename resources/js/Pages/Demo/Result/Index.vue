    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 flex-1">
      ...
      <header class="mb-8 sm:mb-12">
        ...
        <p class="text-base sm:text-lg text-gray-600 dark:text-gray-400 max-w-2xl">
          View comprehensive results from the demo election. Click on any position to see detailed voting breakdown.
        </p>
      </header>
      ...
      >
        <stat-card
          ...
          :aria-label="`Total votes cast: ${final_result.total_votes}`"
        />
        <stat-card
          icon="clipboard-list"
          :value="posts.length"
          label="Positions"
          :aria-label="`Number of positions: ${posts.length}`"
        />
        <stat-card
          icon="calendar"
          :value="new Date().toLocaleDateString()"
          label="Generated Date"
          :aria-label="`Results generated on ${new Date().toLocaleDateString()}`"
        />
        <stat-card
          icon="shield-check"
          :value="mode === 'global' ? 'MODE 1' : 'MODE 2'"
          label="Election Mode"
          :aria-label="`Election running in ${mode === 'global' ? 'Global Demo Mode' : 'Organisation Demo Mode'}`"
        />
      </section>
      ...
      <section class="mb-8 sm:mb-12 flex flex-wrap gap-3">
        <button
          @click="downloadPDF"
          class="inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-700 dark:hover:bg-indigo-800 text-white rounded-lg font-medium transition-colors focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-900 min-h-[44px] sm:min-h-[48px]"
          :disabled="isDownloading"
          aria-label="Download election results as PDF"
        >
          <svg v-if="!isDownloading" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
          </svg>
          <span class="hidden sm:inline" v-if="!isDownloading">Download PDF</span>
          <span class="sm:hidden" v-if="!isDownloading">PDF</span>
          <span v-if="isDownloading">Generating...</span>
        </button>

        <button
          @click="printResults"
          class="inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-gray-600 hover:bg-gray-700 dark:bg-gray-700 dark:hover:bg-gray-800 text-white rounded-lg font-medium transition-colors focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 dark:focus:ring-offset-gray-900 min-h-[44px] sm:min-h-[48px]"
          aria-label="Print election results"
        >
      ...
      <section
        ...
        role="region"
        aria-label="Election results by position"
      >
        <candidate-card
          v-for="post in posts"
          :key="post.post_id"
      ...
      <section v-if="!posts || posts.length === 0" class="text-center py-16 sm:py-20">
        ...
        <p class="text-lg text-gray-500 dark:text-gray-400">No demo election posts found.</p>
        <p class="text-sm text-gray-400 dark:text-gray-500 mt-2">Create demo posts to view results.</p>
      </section>
...
const props = defineProps({
  ...
  page_title: {
    type: String,
    default: 'Demo Election Results'
  }
});

const getPostResults = (postId) => {
  return props.final_result?.posts?.find(p => p.post_id === postId) || {};
};
