      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
          ...
          <div class="flex items-start justify-between gap-4">
            ...
            <Link
              :href="route('organisations.members.import.tutorial', organisation.slug)"
          ...
          <div class="lg:col-span-3">
            ...
            <section v-if="currentStep === 'preview' && preview" class="bg-white rounded-lg shadow-xs p-6 mb-6">
              ...
              <div class="bg-neutral-50 rounded-lg border border-neutral-200 overflow-hidden mb-6">
                    ...
                    <thead class="sticky top-0 bg-neutral-100 border-b border-neutral-200">
                      <tr>
                        <th v-for="header in preview.headers" :key="header"
                            class="px-4 py-2 text-left font-medium text-neutral-700">{{ header }}</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="(row, index) in preview.rows.slice(0, 10)" :key="index"
                          class="border-b border-neutral-200 hover:bg-neutral-100">
                        <td v-for="header in preview.headers" :key="header" class="px-4 py-2 text-neutral-900">
                          {{ row[header] || '-' }}
                        </td>
                      </tr>
                    </tbody>
                ...
                <div v-if="preview.rows.length > 10" class="px-4 py-2 bg-neutral-50 border-t border-neutral-200 text-xs text-neutral-600">
            ...
            <section v-if="currentStep === 'processing'" class="bg-white rounded-lg shadow-xs p-6 mb-6">
              <h2 class="text-xl font-semibold text-neutral-900 mb-4">{{ t.processing.title }}</h2>
              ...
              <div class="mb-6">
                ...
                <div class="w-full bg-neutral-200 rounded-full h-3" role="progressbar" :aria-valuenow="importProgress" aria-valuemin="0" aria-valuemax="100">
              ...
              <div class="grid grid-cols-3 gap-4 text-center text-sm">
                ...
                <div class="bg-primary-50 rounded-lg p-3">
                  <div class="text-2xl font-bold text-primary-700">{{ jobStatus.total_rows || '—' }}</div>
                  <div class="text-primary-600">{{ t.processing.total }}</div>
                </div>
              </div>

              <p class="text-xs text-neutral-500 mt-4 text-center">{{ t.processing.background_hint }}</p>
            </section>
            ...
            <div v-if="error && currentStep !== 'upload'" class="mt-4 p-4 bg-danger-50 border border-danger-200 rounded-lg">
              <p class="text-sm text-danger-700"><strong>{{ t.error.label }}</strong> {{ error }}</p>
              <button @click="resetFile"
                      class="mt-2 text-sm text-danger-600 underline focus:outline-none focus:ring-2 focus:ring-red-400 rounded">
                {{ t.error.try_again }}
              </button>
            </div>
          ...
          <aside class="lg:col-span-1" aria-label="File format information">
            <div class="bg-primary-50 rounded-lg p-6 border border-primary-200 sticky top-4">
              ...
              <div class="space-y-4 text-sm">
                ...
                <div class="border-t border-primary-200 pt-4">
                  <p class="font-medium text-neutral-900 mb-2">{{ t.info_panel.scale_heading }}</p>
                  <p class="text-neutral-600">{{ t.info_panel.scale_text }}</p>
                </div>
