      ...
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="space-y-8">
          ...
          <section class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <p class="text-gray-700 mb-3">{{ t.section_requirements.intro }}</p>
            <ul class="mb-5 space-y-1">
              <li v-for="(req, idx) in t.section_requirements.required" :key="idx"
                  class="flex items-start gap-2 text-sm text-gray-800 font-medium">
                <span class="text-green-600 mt-0.5">★</span>{{ req }}
              </li>
            </ul>
            <p class="font-medium text-gray-800 mb-2">{{ t.section_requirements.optional_heading }}</p>
            <ul class="space-y-1">
              <li v-for="(opt, idx) in t.section_requirements.optional" :key="idx"
                  class="flex items-start gap-2 text-sm text-gray-600">
                <span class="text-blue-400 mt-0.5">•</span>{{ opt }}
              </li>
            </ul>
          </section>
          ...
          <section class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <ul class="space-y-2 mb-5">
              <li v-for="(rule, idx) in t.section_file_format.rules" :key="idx"
                ...
                <span class="text-blue-500 mt-0.5">•</span>{{ rule }}
              </li>
            </ul>

            <p class="font-medium text-gray-800 mb-2">{{ t.section_file_format.example_heading }}</p>
            <pre class="bg-gray-900 text-green-400 rounded-lg p-4 text-sm font-mono overflow-x-auto mb-4 whitespace-pre-wrap">{{ t.section_file_format.example }}</pre>
          ...
          <section class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <p class="text-gray-700 mb-4">{{ t.section_upsert.body }}</p>
            <div class="space-y-3">
              <div v-for="(item, idx) in t.section_upsert.items" :key="idx"
                   class="flex gap-4 p-4 rounded-lg border border-gray-100 bg-gray-50">
                <span class="flex-shrink-0 w-6 h-6 rounded-full bg-blue-600 text-white text-xs font-bold flex items-center justify-center mt-0.5">
                  {{ idx + 1 }}
                </span>
                <div>
                  <p class="font-medium text-gray-900 text-sm mb-0.5">{{ item.label }}</p>
