      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-3 gap-4 mb-6">
          <div class="bg-white rounded-lg border border-gray-200 p-4 text-center">
            ...
            <div class="text-xs text-gray-500 mt-1">{{ t.type_staff }}</div>
          </div>
          <div class="bg-white rounded-lg border border-gray-200 p-4 text-center">
            <div class="text-2xl font-bold text-gray-600">{{ stats.guests }}</div>
            <div class="text-xs text-gray-500 mt-1">{{ t.type_guest }}</div>
          </div>
          <div class="bg-white rounded-lg border border-gray-200 p-4 text-center">
            <div class="text-2xl font-bold text-violet-600">{{ stats.election_committee }}</div>
            <div class="text-xs text-gray-500 mt-1">{{ t.type_election_committee }}</div>
          </div>
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t.col_type }}</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t.col_role }}</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t.col_assigned }}</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t.col_expires }}</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-if="filteredParticipants.length === 0">
                <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-400">{{ t.empty }}</td>
              </tr>
              <tr v-for="p in filteredParticipants" :key="p.id" class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ p.role || '—' }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatDate(p.assigned_at) }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  <span v-if="p.is_expired" class="text-red-500 font-medium text-xs">{{ t.expired }}</span>
                  <span v-else-if="p.expires_at" class="text-gray-500 text-xs">{{ formatDate(p.expires_at) }}</span>
