...
# 5. Adequacy is NEARLY invariant where realization is not — equivalence claim withdrawn by audit
...
> ## ⚠️ CORRECTED BY AUDIT `KR-REP-REDUCTION-AUDIT-2026-09` — the equivalence claim is REFUTED
>
> **Both the original wording (*"carries no information about `D`"*) and its review-supplied
> replacement (*"information-equivalent recodings"*) are FALSE.** Check `2.3` disproved the assumed
> relation `desc = (n+1) − asc`, and the partition test then disproved equivalence outright:
>
> ```
> asc → desc well-defined ?   NO — 21 369 violations
> desc → asc well-defined ?   NO — 20 873 violations
> SAME PARTITION on cases ?   NO
> N_viol            5 530 777  (asc)   vs   5 577 078  (desc)
> ```
>
> **Cause — the tie-break.** Ascending sorts by `(v, i)`, descending by `(−v, i)`; **both give the
> earlier index the lower rank among equals**, so descending is *not* the reversal of ascending
> whenever values tie. Minimal witness: `R3 = [23000, 46000, 46000]` → `asc = [1,2,3]`,
> `desc = [3,1,2]`, while `(n+1)−asc = [3,2,1]`.
>
> ### What survives, at the strength the evidence supports
>
> `[EXP]` **Two conventions differing only in an ordering choice yield adequacy that is NEARLY
> identical (`Ĥ` 3.3937 vs 3.3965; `N_viol` within 0.8 %; `|image|` 162 both) while the fixed
> decoder's argmax accuracy swings 54 points.**
>
> **`ADEQUACY` is nearly invariant; `REALIZATION` is not. That separation stands.**
>
> ⚠️ **WITHDRAWN: that the swing is PURELY a decoder effect.** The partitions differ, so the two
> representations carry **different information** — a small part of the swing is representational, and
> the experiment cannot apportion it.
>
> ### And the more interesting finding the audit produced
>
> > **The tie-break rule — a choice that looks purely presentational — is ENTANGLED with the ordering
> > direction and thereby becomes EVIDENTIAL.**
>
> A convention adopted for determinism turned out to carry information. **This is a sharper instance
> of the programme's own theme than the one I originally claimed**, and it was invisible until an
> audit tested the invertibility assumption rather than assuming it.

**Why this matters.** The load-bearing question was whether the map between conventions is
**invertible**. **It is not** — §2.3 of the audit tested it instead of assuming it, and found 21 369
counterexamples. **The two `R2` representations are therefore NOT `Q`-equivalent to each other**, and
`DECISION-01` does not apply to this pair. **The design's §0.2 rule — never assert equivalence without
proving it — is exactly what this violation would have breached had the audit not run.**

**This is NOT `DECISION-01` in a new setting** — that reading required an invertible recoding, and the audit refuted invertibility. What it is instead: — and it locates the sensitivity precisely: the
