...
// ✅ FIXED: Apply guest middleware only where needed to prevent redirect loops
Route::get('/login', [\App\Http\Controllers\Auth\LoginController::class, 'show'])->name('login')->middleware('guest');
Route::post('/login', [\App\Http\Controllers\Auth\LoginController::class, 'store'])->middleware('guest');

Route::get('/register', [\App\Http\Controllers\Auth\RegisterController::class, 'show'])->name('register')->middleware('guest');
Route::post('/register', [\App\Http\Controllers\Auth\RegisterController::class, 'store'])->middleware('guest');

// Password reset routes (allow guests to reset password)
...
// ============================================================================
// EMERGENCY DASHBOARD (Level 2 Fallback)
// ============================================================================
// Used when normal dashboard resolution fails
// Requires authentication but minimal database queries
Route::middleware(['auth'])->group(function () {
    Route::get('/dashboard/emergency',
        [App\Http\Controllers\EmergencyDashboardController::class, 'index']
    )->name('dashboard.emergency');

    Route::post('/dashboard/emergency/logout',
        [App\Http\Controllers\EmergencyDashboardController::class, 'logout']
    )->name('dashboard.emergency.logout');

    Route::get('/dashboard/emergency/health',
        [App\Http\Controllers\EmergencyDashboardController::class, 'healthCheck']
    )->name('dashboard.emergency.health');
});
