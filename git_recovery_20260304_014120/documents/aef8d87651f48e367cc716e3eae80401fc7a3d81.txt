...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    /**
     * Boot the model - assign users to platform organisation on registration
     * ✅ NEW: Use is_platform flag instead of slug to find platform org
     */
    protected static function boot()
    {
        parent::boot();

        // When creating/updating a user, assign to platform organisation (id=1)
        static::creating(function ($model) {
            if (!$model->organisation_id || $model->organisation_id === 0 || $model->organisation_id === '0') {
                // ✅ FIX: Look for is_platform = 1 instead of slug = 'platform'
                ...
                if ($platformOrg) {
                    ...
                    // This is a safety net in case is_platform query fails
