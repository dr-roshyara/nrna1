...
class ElectionOfficerController extends Controller
{
    ...
    public function index(Organisation $organisation)
    {
        ...
        $officers = ElectionOfficer::with('user:id,name,email', 'appointer:id,name')
    ...
    public function store(Request $request, Organisation $organisation)
    {
        ...
        $request->validate([
            'user_id' => [
                ...
                function ($attribute, $value, $fail) use ($organisation) {
                    $exists = ElectionOfficer::where('user_id', $value)
                        ->where('organisation_id', $organisation->id)
                        ->whereNull('deleted_at')
                        ->exists();

                    if ($exists) {
                        $fail('This user is already an election officer for this organisation.');
                    }
                },
            ],
