...
public function store(Request $request)
{
    ...
    \Log::info('CodeController store method started', [
        ...
        'url' => $request->fullUrl(),
        'is_ajax' => $request->wantsJson() || $request->expectsJson(),
    ]);

    // Early debug - let's see if we get this far
    if ($request->wantsJson() || $request->expectsJson()) {
        \Log::info('AJAX request detected in store method');
    }
    ...
    \Log::info('Validating input', [
        'voting_code' => $request->input('voting_code'),
        'all_input' => $request->all(),
    ]);
    ...
    if ($code->can_vote_now == 1) {
        ...
        if ($request->wantsJson() || $request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'You already have an active voting session. Please continue with the voting process.',
...
if (isset($auth_user->client_ip) && !empty($auth_user->client_ip)) {
    ...
    try {
        ...
        // ✅ SUCCESS: Handle both AJAX and regular requests
        $route = $voterSlug
            ? 'slug.code.agreement'
            : 'code.agreement';
        $routeParams = $voterSlug ? ['vslug' => $voterSlug->slug] : [];
        $redirectUrl = route($route, $routeParams);

        // For AJAX requests, return JSON with redirect URL
        if ($request->wantsJson() || $request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => '✅ Code verified successfully! Please read and accept the voting agreement to continue.',
                'redirect' => $redirectUrl,
                'step' => 2,
                'current_step' => $voterSlug ? $voterSlug->fresh()->current_step : 2,
            ]);
        }
        ...
        $errorMessage = 'System error occurred after code verification. Please try again or contact support.';
        ...
        if ($request->wantsJson() || $request->expectsJson()) {
            return response()->json([
                ...
                'errors' => ['voting_code' => $errorMessage]
            ], 500);
        }

        // For regular form submissions, use back with errors
        return back()->withErrors([
            'voting_code' => $errorMessage
        ])->withInput();
    }
}
...
private function validateExistingCodeModel($code, $auth_user, $voterSlug = null)
{
    ...
    if ($code->has_code1_sent == 1 && $code->is_code1_usable != 1) {
        ...
        // If can't auto-reset, check if code was successfully verified before
        if ($code->can_vote_now == 1) {
            // Code was already successfully verified, allow proceeding to agreement
            ...
            // Code was used but verification failed, force regeneration
            \Log::warning('Code was used but verification unsuccessful, forcing regeneration', [
                'user_id' => $auth_user->id,
                'can_vote_now' => $code->can_vote_now,
                'is_code1_usable' => $code->is_code1_usable,
            ]);
            ...
            return redirect()->route($route, $routeParams)
                ->with('error', 'Code verification was incomplete. Please request a new code.');
        }
    }
