...
final class SnapshotIntegrityHashTest extends TestCase
{
    ...
    public function test_from_snapshot_produces_non_empty_hash_string(): void
    {
    ...
    public function test_verify_passes_for_intact_snapshot(): void
    {
        $snapshot = $this->makeSnapshot();
        $hash = SnapshotIntegrityHash::fromSnapshot($snapshot);

        $this->assertTrue($hash->verify($snapshot));
    }

    public function test_verify_fails_for_tampered_snapshot(): void
    {
        $snapshot = $this->makeSnapshot();
        $hash = SnapshotIntegrityHash::fromSnapshot($snapshot);
