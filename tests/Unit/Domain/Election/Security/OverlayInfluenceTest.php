...
class OverlayInfluenceTest extends TestCase
{
    ...
    {
        $this->assertEquals('continue_unchanged', OverlayInfluence::CONTINUE_UNCHANGED->value);
    }

    public function test_trust_elevation_request_has_correct_string_value(): void
    {
        $this->assertEquals('trust_elevation_request', OverlayInfluence::TRUST_ELEVATION_REQUEST->value);
    }

    public function test_require_re_verification_has_correct_string_value(): void
    {
        $this->assertEquals('require_re_verification', OverlayInfluence::REQUIRE_RE_VERIFICATION->value);
    }

    public function test_require_constitutional_review_has_correct_string_value(): void
    {
        $this->assertEquals('require_constitutional_review', OverlayInfluence::REQUIRE_CONSTITUTIONAL_REVIEW->value);
    }

    public function test_trust_evaluation_inconclusive_has_correct_string_value(): void
    {
        $this->assertEquals('trust_evaluation_inconclusive', OverlayInfluence::TRUST_EVALUATION_INCONCLUSIVE->value);
    }
    ...
    public function test_all_enum_values_exclude_banned_vocabulary(): void
    {
        ...
        foreach (OverlayInfluence::cases() as $case) {
            foreach ($bannedTerms as $term) {
                $this->assertStringNotContainsString($term, $case->value,
                    "OverlayInfluence::{$case->name} value '{$case->value}' contains banned term '$term'");
            }
        }
    }
}
