...
| ⭐ **RD-7** *(Run 2 addendum — first ANOMALY)* | **A recommendation CAN currently be decided twice**: two identical ACCEPTED records exist for REC-eaf245474a *(06:30 and 15:11 — a session interruption re-ran the command)*. §3's abstract invariant question *("can recommendations be re-decided?")* is **answered by accident: yes, unguarded** — and the dashboard's naive record-count visibly skews *(decided=2 · accepted=2 for ONE unique recommendation)*. *The domain likely wants exactly-once or last-wins — but WHICH is a domain decision: a legitimate re-decision after conditions change is RD-4's revision question wearing different clothes* | **1 real duplicate** | — | **OBSERVED** | ⛔ **observe — do not patch the guard before the semantics are decided** *(patching now would encode exactly-once without evidence that last-wins isn't the domain's truth)* |
...
| ⭐ **ADDENDUM (15:11): the duplicate-decision anomaly (RD-7)** — the first live evidence that **Decision may carry its own invariants** *(idempotency? supersession?)*, which weighs toward Recommendation-being-more-than-transformation. Also: the 06:30/15:11 pair gives the first REAL re-decision interval datum, free of charge | `decisions.jsonl` — 4 lines, 1 unique recommendation |
