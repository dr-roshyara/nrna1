...
| **Platform Bootstrap** *(full capability: install + configure + verify + report. ⭐ Check-before-create finding: INSTALL already exists — husky's npm `prepare` configures hooks on every `npm install`; version-controlled `.husky/` ships the trigger. The VERIFY slice — `doctor` — is IMPLEMENTED, earned by the 2026-08-04 incident: the hook was active nowhere until manually diagnosed)* | husky `prepare` (install) + `php scripts/observations/doctor.php` (verify — 8 checks, exit-code gating) | a bootstrap CLI beyond these two: a SECOND developer actually onboarding, or a non-npm environment needing the hooks | onboarding friction observed, not assumed | **PARTIALLY ACTIVE** (doctor live · install pre-existing · CLI gated). ⛔ CI/IDE auto-install stays out per the review's own caution — separate trigger instrumentations, each evidence-gated |
