...
| **Platform Bootstrap** *(v1.0 model per review 2026-08-04 — THREE use cases: **Initialize Repository · Verify Installation · Upgrade Installation**. ⭐ Check-before-create finding: INSTALL for THIS repo already exists — husky's npm `prepare`; the VERIFY slice — `doctor` — is IMPLEMENTED, earned by the hook incident. The reviewer's own scoping accepted: v0.5 today; v1.0 = initializing a FRESH repository from scratch)* | husky `prepare` (this repo's install) + `php scripts/observations/doctor.php` (verify — 8 checks, exit-code gating) | **Initialize + Upgrade: a SECOND repository actually adopting KnowledgeOS** *(converges with THE second-adopter gate + the platform-extraction row — "initialize a fresh repository" presupposes a fresh repository exists to initialize)* · local-CLI convenience: a second developer onboarding with observed friction | a real adopting repository, not a synthetic one | **PARTIALLY ACTIVE** (verify live · install pre-existing · init/upgrade gated on the second adopter). ⛔ CI/IDE auto-install stays out. **BootstrapPort** *(install/verify/upgrade/report — PythonBootstrapAdapter etc.)* staged with the DecisionCaptureService trigger-shape: the port is defined when a SECOND implementation or consumer arrives; one implementation needs no port |
