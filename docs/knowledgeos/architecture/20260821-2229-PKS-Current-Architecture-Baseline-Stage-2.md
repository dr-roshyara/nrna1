...
| .claude/settings.json permission mapping | 1 allow / 1 ask / 1 deny block present (measured tier 1); the Runtime Adapter record at issuance described 19 deny / 22 ask / 8 allow | **F** — the record's stated counts do not match the current file |
...
11. **Documentation/runtime delta on the permission adapter** (tier 1 vs tier 5): the record states 19 deny/22 ask/8 allow; the current `.claude/settings.json` has 1 allow/1 ask/1 deny.
...
| **U-12** | Does the current `.claude/settings.json` (1 allow/1 ask/1 deny) reflect the Runtime Adapter record (19/22/8)? | tier 1 vs tier 5 | counts differ; which is current is unknown |
...
| **X-07** | **Permission-adapter counts**: Runtime Adapter record states 19 deny/22 ask/8 allow; current `.claude/settings.json` has 1 allow/1 ask/1 deny. | tier 5 vs tier 1 | CONTRADICTED |
