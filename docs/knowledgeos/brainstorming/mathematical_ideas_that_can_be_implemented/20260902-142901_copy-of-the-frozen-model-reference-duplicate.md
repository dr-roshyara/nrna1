...
`Factivity` · `Contr` and the fourth value · `⪰` · `≡_sem` · `δ` · the retirement/lifecycle relation ·
...
**Queue, unchanged:** `Factivity → Contr → ⪰`. `N_eff` and `⪰` are **related but independent**.
...
* **To extend the programme:** the queue is `Factivity → Contr → ⪰`, and the first two are
