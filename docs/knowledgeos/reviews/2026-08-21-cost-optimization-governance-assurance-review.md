...
> ## \U0001F534 **The trend is the finding: 29 % → 46 % → 58 %.**
...
| | Risk | Mitigation |
|---|---|---|
| \U0001F534 | **False assurance** — a GREEN mechanical report reads as *"the design is sound"* | ⭐ **the report must state what it did NOT check, positively.** `X-5`'s missing-act blindness is the specific case, and it is the class that produced `RD-1`, `RD-7` and `RD-3·b` |
| \U0001F534 | **Scope creep from checker to gate** — a warn-only tool quietly becomes a blocker | **Phases 2–4 each name their authority.** A gate without a PO/ARB act is an unauthorized governance object |
