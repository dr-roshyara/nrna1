# `KOS-OPERATING-MODEL-001` — implementation appointment **RECORDED** (identity placeholder) · FINAL GOVERNANCE + COMMUNICATION OPERATING MODEL
...
**Act:** PO/ARB 2026-08-22 — verbatim: *"I confirm `KOS-OPERATING-MODEL-001` as the work-item name and authorize delivery of all three layers: operating model, minimal supporting capability, and verification/acceptance. Then appoint a fresh independent implementation session."*
**Commissioned prompt:** `docs/knowledgeos/reviews/2026-08-22-KOS-OPERATING-MODEL-001-implementation-prompt.md` (verbatim, 40 sections)
...
| Decision | Value |
|---|---|
| Work-item name | **CONFIRMED: `KOS-OPERATING-MODEL-001`** (no longer provisional) |
| Layer scope | **ALL THREE LAYERS AUTHORIZED**: operating model · **minimal supporting capability** · verification/acceptance |
| Role appointed | **Architecture implementation** (fresh, independent) |
| Scope | implement the capability **according to the recorded implementation prompt** (verbatim artifact) + the authorized three-layer scope |
...
**On "minimal supporting capability" (PO/ARB's scoping signal, recorded):** the capability layer is **minimal**. The operating model is the primary deliverable; it **consumes the existing `AST-018` commands** as the canonical mechanism and does **not** re-implement, enlarge, or re-open them. Any new software produced under this commission is minimal support for the operating model — never a second workflow engine, never a re-implementation of AST-015/AST-017/AST-018. **Verification/acceptance** in scope = the §36 testing + §38 completion-report deliverable (self-verification and readiness for independent verification) — **not** adoption, which remains the governance path (prompt §38: IMPLEMENTED ≠ VERIFIED ≠ ADOPTED ≠ AUTHORIZED).

## 2 · Governed state of the work item (read-only bootstrap, 2026-08-22)

`php .claude/scripts/session-bootstrap.php --work-item=KOS-OPERATING-MODEL-001 --process-label=<this session> --json`
...
```
PO/ARB names work item + authorizes three-layer scope + appoints fresh implementation session   ✅ (this act)
...
session_completion:
  ...
  completed_work:    # recorded the PO/ARB act (name confirmed + all-three-layers scope authorized + fresh
                     #   implementation session appointed); captured the governed state (bootstrap
                     #   UNRESOLVABLE — no record yet, fail-closed); recorded the sequence to implementation
                     #   + identity bars + layer-scope meaning ("minimal supporting capability"); placement derived
  evidence:          # PO/ARB act 2026-08-22 (verbatim); AST-017 bootstrap JSON (UNRESOLVABLE,
                     ...
                     #   registration; implementation-prompt artifact (verbatim, 40 sections)
  open_items:        # appointee identity not declared; workflow record not created; lane not registered;
                     #   capability not implemented/adopted; AST-018 slice still needs STOP → verification
                     #   lane → adoption; FU-1/FU-2 classification still open on the AST-018 slice
...
next_actor:
  ...
  reason:            # the PO/ARB appointed a fresh independent implementation session for all three layers;
                     #   identity must be declared by the appointee and verified through the mechanism
                     #   (INV-ATTR-1/2); REGISTER/HANDOFF/START are governed + manual (G-3; boundary ruling);
                     #   this process is not the appointee and does not self-appoint (P-3; EP-02/R-34)
...
**Traceability:** PO/ARB act 2026-08-22 (verbatim) · commission registration (`…-KOS-OPERATING-MODEL-001-commission-registration.md`) · implementation prompt (`…-KOS-OPERATING-MODEL-001-implementation-prompt.md`, verbatim, 40 sections) · governance review (in-session, Option B) · adopted six-role operating model (2026-08-19) · `KOS-NEXT-ACTOR-ORCHESTRATION-001` / `AST-018` (separate slice, unchanged) · AST-017 bootstrap (UNRESOLVABLE — no record) · AST-015 · `G-3` · `P-3` · `EP-01` · `EP-02`/`R-34` · `INV-ATTR-1/2` · `ES-004.3` · `F1` · placement: `scripts/doc-placement.php` (product-specific · knowledgeos → `docs/knowledgeos`, exit 0)
