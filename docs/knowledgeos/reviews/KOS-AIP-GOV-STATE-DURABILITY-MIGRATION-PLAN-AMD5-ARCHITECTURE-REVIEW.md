...
# 18 · Reviewer's note on the **AMD6 commission** *(added 2026-08-21, additive — no verdict in §1–17 is changed)*

⛔ **This section proposes no plan text and authors no remedy.** It records five defects in the AMD6 commission as drafted, because each would make AMD6 ship a new instance of a defect it is chartered to fix. **`ccf6c9c7` is ineligible to author AMD6** *(it owns the findings AMD6 remedies)* **and is on the exclusion ledger for the AMD6 review.**

| | Commission item | Defect, and the direction that is safe |
|---|---|---|
| 🔴 **`C-1`** | **item 7 — `DI-7` naming** | ⭐ **The example renames the WRONG family.** §6's `CASE A`/`CASE B` are cited in the Governance review, the AMD3 review, the AMD4 review, this review and **§10 criterion 10**; `CASE α`/`CASE β` were introduced by AMD5 and are cited nowhere outside the plan. **Renaming §6 would invalidate citations in four delivered reviews — the exact trap `DI-1`'s remedy was right to avoid when it renumbered the AMD4 additions and preserved the originals.** ⇒ **rename the NEWER family only**, e.g. `PRE-SWITCH DISPOSITION` / `POST-DEMOTION DISPOSITION`, with *"AMD5 called these CASE α/β"* labelled |
| 🔴 **`C-2`** | **items 3 and 4 disagree on granularity** | item 3: *"Phase 7 MUST NOT remove **the affected evidence**"* *(record-level)* · item 4: *"MUST NOT remove **a store** containing an undisposed quarantine"* *(store-level)*. ⛔ **Record-level retention requires inventing partial-removal semantics and a marker state for a partially-removed store, against `RC-6`'s directory-level rule.** ⇒ **choose STORE-LEVEL**: the demoted store and its marker are retained entire until governance disposition |
| 🔴 **`C-3`** | **item 4 — quarantine destination** | Two mechanics the commission gestures at but does not name. **(a)** a quarantined record **is** a `*.json` file, so the destination must sit outside **every** directory scanned by the `glob($recordDir.'/*.json')` predicate (`session-resolve.php:130`, and §4's Phase-1 predicate) or it will be read as a work item. **(b)** §4.5 currently states **exactly two markers**; a governed-but-non-authoritative quarantine store needs a **third** *(`QUARANTINED`)*, and §4.5's *"the two markers"* row becomes stale the moment item 4 lands |
| 🔴 **`C-4`** | **item 1 — the order being frozen** | ⭐ **Phase 5's own bookkeeping is a gap-window write BY CONSTRUCTION, and freezing the order without deciding where it lands ships the defect again.** Until step 3, `P-1` still resolves to **runtime** ⇒ the re-hash evidence, the `1b` reconciliation record and the reader-switch evidence all land in the store just measured at step 1. ⇒ **the Phase-7 final re-hash then reports a delta in the NORMAL case, and criterion 12's *"identical"* branch is unreachable — `RD-10`'s exact degradation, recurring at the irreversible gate.** ✅ **Safe direction, and it is sequencing rather than redesign: Phase 5's own evidence is recorded AFTER the writer switch, into the new store** — which also makes the switch-over record `RD-3` relies on the **first authoritative record in the new store** rather than the last non-authoritative one in the old |
| ⚠️ **`C-5`** | **item 6 — `DI-6`'s stale rows** | The prescribed text says `OPEN-M6` spans **AMD3 + AMD4 + AMD5**. ⛔ **That is stale the moment AMD6 lands** — the row must read **AMD3 … AMD6**, or `DI-6` ships a fresh instance of the staleness it is chartered to remove |
| ✅ | **item 1 — step `1b`** | The required form is correct and also closes `RD-1`. ⚠️ **State that `1b` has THREE outcomes, not one** — no delta · delta ⊆ declared *(reconcile)* · delta ⊄ declared *(🔴 **freeze violation ⇒ STOP before any switch**)*. `1b` is therefore also the last point at which Phase 5 can be abandoned cheaply |

⭐ **`C-4` is the one that matters most, because item 1 freezes the Phase-5 sequence and item 3 gates on the Phase-7 comparison — so the choice is made either deliberately or by omission.** ⛔ **None of the five requires a decision, a new bounded context, or any change to `B′`, `R-CONFLICT`, `INV-ORDER` or `OPEN-M3`; `OPEN-M1`/`M2`/`M4`/`M5`/`M6` stay open and undecided here.**
