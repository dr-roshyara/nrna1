...
| [EKS-12](EKS-12-theory-governance-scope-gap.md) | No governance authority has evidenced scope over **KnowledgeOS theory** | **BACKLOG · GOVERNANCE SCOPE QUESTION** | governance-scope problem |
| [EKS-12](EKS-12-out-of-root-evidence-admission-mechanism.md) | No formal mechanism for admitting out-of-corpus-root evidence into a research programme — three instances so far, each reinvented from scratch | BACKLOG | operating-model problem |
...
**EKS-12 added 2026-09-08** from the `three_model_convergence` research programme's own MD-034 characterization study — a recurring operating-model gap (no reusable admission procedure for out-of-corpus-root evidence), independently converged on across three separate instances (`03`/`04`, `06`, and a recommended `12-randomized-results.md`). Checked against `EKS-06` before filing (`ES-005.4`, never a copy) — a different problem in a different subsystem (a research-corpus admission workflow vs. `EKS-06`'s Track-2 intra-document reference checker), not a duplicate.
