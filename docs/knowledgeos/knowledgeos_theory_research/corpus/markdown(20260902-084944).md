...
```math
\boxed{ ZeroLens(K,I,\Gamma,L)\rightarrow B }
```
...
```math
B \rightarrow Investigation/Evaluation/Action \rightarrow K_{t+1}.
```
...
```math
K_t \rightarrow Zero \rightarrow Boundary \rightarrow Investigation \rightarrow Evaluation \rightarrow K_{t+1} \rightarrow Zero
```
...
```math
K_{t+1}\neq K_t \Rightarrow Zero(K_{t+1})\text{ may be required}.
```
...
```math
TryBest = \text{maximize justified epistemic expansion}
```
...
```math
\boxed{ \text{Do not collapse semantically distinct conditions merely because they share a coarse projection.} }
```
...
```math
b_1\neq b_2 \quad\text{but}\quad \pi(b_1)=\pi(b_2)=U.
```
...
```math
\boxed{ \text{preserve }B \quad\text{before applying }\pi. }
```
...
```math
\boxed{ ZeroLens\text{ may produce structured boundary information} }
```
...
```math
\boxed{ ZL(K_t,I_t,\Gamma_t,L_t) \rightarrow B_t }
```
...
```math
\boxed{ Boundary \;\text{contains more information than}\; U }
```
...
```math
\boxed{ \textbf{KR-ZERO-2026-09-02-H — Boundary Separation Experiment} }
```
...
```math
\boxed{ (K,I,\Gamma,L) \rightarrow (B,\rho,\pi) }
```
