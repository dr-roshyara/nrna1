Understood. I will execute the ADR-3 controlled revision cycle as specified.

## Round 37-03R — ADR-3 Revision Cycle

**Mission:** Revise ADR-3 for precision, implementation neutrality, and drift resistance. No constitutional decisions change. Five required revisions only.

---

Let me begin by re-reading the current ADR-3 document to identify all locations requiring revision.

<read filepath="docs/architecture/design/Round37-03_ADR-3_Evidence_and_Verifier_Architecture.md">

The document has been read. I have identified all locations requiring revision per R1–R5. Now applying revisions systematically.

---

## Applying Revisions

### R1 — AC-31 Realization Discipline

**Locations identified for correction:**

1. **Part C.3 (line 253):** "Selected option for EH-01: Option B" — needs scope discipline note added
2. **Part C.4 (lines 261–263):** VO-3 statement implies external repository as the reference form — needs neutralization
3. **Part D.2 Option B (line 301):** "repository is the independent reference standard" — needs qualification
4. **Part H.2 (line 583):** "centrally held external repository" — needs neutrality

**New section to insert after Part C.4:**

### R2 — VO-3 Qualification Strengthening

**Part F.4 (lines 463–489) needs revision.** The current text states VO-3 is a "CONFIRMED CANDIDATE" but the delivery independence argument drifts toward implying sufficiency.

### R3 — Independent Reference Legitimacy Observation

**New observation to add after Part F or in Part G.**

### R4 — Certification Dependency Strengthening

**Part I.3 (lines 636–639) needs strengthening.**

### R5 — Three-Stratum Protection Invariant

**Part E (after E.2) needs invariant insertion.**

---

Now writing the revised document.

<create-file path="docs/architecture/design/Round37-03_ADR-3_Evidence_and_Verifier_Architecture.md" content="# Round 37-03 — ADR-3: Evidence and Verifier Architecture

**Program:** NRNA DDD Trustworthiness Research Program
**Round:** 37 — ADR Authoring
**Document:** ADR-3 of 7
**Status:** REVISED — SUBMITTED FOR FINAL ARB APPROVAL
**Revision:** 37-03R — Five required revisions applied
...
## Revision Notes (37-03R)

This revision applies five required revisions to the ADR-3 document approved with conditions on June 15, 2026. No constitutional decisions are changed. Revisions add precision, implementation neutrality, and architectural drift resistance.

| Revision | Description | Locations |
|----------|-------------|-----------|
| R1 | AC-31 realization discipline — independence ≠ external organization | C.4 (new section), D.2, H.2, throughout |
| R2 | Stronger VO-3 qualification — custody vs. origin independence | F.4 |
| R3 | Independent Reference Standard legitimacy observation — OBS-ADR3-01 | New section G.5 |
| R4 | Certification dependency strengthening — independent access required | I.3 |
| R5 | Three-Stratum non-collapse invariant — ADR3-INV-01 | E.2 (new subsection) |
...
- Constitutional significance: VO-3 delivered to the voter is the strongest existing candidate for an independent reference anchor — it represents a reference standard held outside the election system's control. However, its independence is custody-based (post-delivery), not origin-based (pre-delivery). This distinction is constitutionally significant — see Part F.4.
...
**Realization note:** Option B states the constitutional requirement: the reference standard must be constitutionally independent. It does not specify HOW that independence is realized. An external organization holding the reference is one possible realization. Cryptographic anchoring where the reference is published to a location the system cannot retroactively modify is another. A distributed reference held by voters (such as VO-3 receipts) is a third. Option B does not select among these — it establishes the constitutional property they must all satisfy.
...
Key qualification: if the commitment is published by the election system to its own infrastructure, Option C degenerates to Option A. Constitutional independence of the commitment publication holder is the load-bearing requirement — the same constitutional grounding challenge as any independent reference realization.
...
**Verdict:** Constitutionally valid if commitment publication holder is independently constitutionally grounded. Constitutionally equivalent to Option B in constitutional structure. Does not address Gap A-3 (completeness) independently. VO-1 compatibility requires assessment at implementation level (Round 38+).
...
### C.4 AC-31 Realization Discipline

**Constitutional requirement vs. implementation form:**

AC-31 establishes a constitutional requirement: the reference standard used for audit-grade evidence authenticity verification must be constitutionally independent of the election system that produced the evidence.

AC-31 does NOT require:
- An external organization
- An external company
- A separate bounded context
- A separate microservice
- A separate deployment unit
- A particular technical mechanism (cryptographic commitment, external repository, distributed ledger)

These remain implementation choices for Round 38+ design.

The constitutional question is: is the reference standard outside the election system's unilateral control? The answer determines AC-31 compliance. How that independence is achieved — through organizational separation, cryptographic anchoring, voter-held receipts, or other mechanisms — is a design decision, not a constitutional one.

This discipline mirrors ADR-2's governing warning: constitutional requirement ≠ organizational prescription. The same discipline applies to evidence architecture.

### C.5 What AC-31 Constrains
...
3. VO-3 (ReceiptHash) is a CANDIDATE for per-vote independent reference anchoring — custody independence after delivery is established; origin independence and AC-31 sufficiency are unresolved (see Part F.4)
...
**Assessment:**
- AC-29: satisfied — external repository can hold reference; authenticity verified against repository, not originating system
- AC-31: satisfied if the repository is constitutionally independent of the election system. Note: "repository" here describes a constitutional relationship (evidence held outside system control), not a specific technical storage mechanism.
...
- Governance complexity: high — the reference holder (whatever form it takes) requires its own constitutional grounding
- Does not structurally separate Completeness from Presence + Authenticity — the reference holder may handle presence AND authenticity reference; completeness is a scope definition question that requires additional constitutional specification
...
- AC-31: satisfied if commitment is externally anchored — the published commitment is the independent reference
...
Options A fails AC-31. Option B satisfies AC-31 but conflates Presence and Authenticity — a single reference holder may hold both the evidence and the reference standard; the constitutional distinction between OBS-36C-03-G (Authenticity ≠ Completeness) and AC-29 (Presence ≠ Authenticity) is maintained by governance structure rather than architectural structure. Option C is an implementation approach for the Authenticity Stratum, not a complete evidence architecture — it does not address Completeness separately.
...
**Discipline note:** Option D's selection does not specify the implementation of any stratum. The Authenticity Stratum's independent reference standard may be realized through a constitutionally independent reference holder (Option B pattern), cryptographic commitment (Option C pattern), voter-held receipts (VO-3), or a combination. Those are Round 38+ design decisions. ADR-3 establishes that each stratum must satisfy its constitutional requirement — it does not specify the technical realization.
...
#### ADR3-INV-01: Three-Stratum Non-Collapse Invariant

> **Completeness, Presence, and Authenticity are constitutionally distinct dimensions. No future architecture may satisfy Completeness by Presence, Presence by Authenticity, or Authenticity by Completeness. Each stratum requires independent constitutional evaluation.**

**Constitutional basis:**
- Completeness ≠ Presence: a record's existence does not establish that it was required (TC1-DI-01: absence of evidence ≠ evidence of absence)
- Presence ≠ Authenticity: a record being present does not establish it is unmodified (TC2-DI-01: presence ≠ truth)
- Authenticity ≠ Completeness: a record being verified authentic does not establish the set is complete (OBS-36C-03-G)

**Enforcement:** Any future architecture proposal that claims to satisfy one stratum through another stratum's verification is constitutionally invalid. An audit report that verifies authenticity of present records has NOT verified completeness. A completeness check that confirms expected records are present has NOT verified their authenticity. These are independent constitutional evaluations.

**Placement:** This invariant is placed here, in the stratum definition section, because it constrains all future evidence architecture design. ADR-4 through ADR-7 and all Round 38+ designs must respect this invariant.
...
**Custody independence vs. origin independence:**

VO-3 demonstrates **custody independence after delivery**: once delivered to the voter, the voter's receipt copy is outside the election system's authority boundary. The election system cannot unilaterally modify what the voter holds. If the voter's VO-3 differs from what the election system claims, the voter can present their receipt as evidence of discrepancy.

VO-3 does NOT automatically demonstrate **origin independence**: the election system generates the receipt, the election system delivers the receipt, and the election system stores a copy of the receipt. All pre-delivery stages are system-controlled.

**The constitutional question for AC-31:** Is custody independence (post-delivery) sufficient to constitute constitutional independence of the reference standard, or does AC-31 require origin independence (the reference standard is generated or held outside the system from inception)?

**ADR-3 determination for VO-3:**

> **VO-3 (voter-held ReceiptHash) is a CONFIRMED CANDIDATE for the per-vote authenticity reference in the Authenticity Stratum.** Custody independence after delivery is established and constitutionally significant. Whether custody independence alone satisfies AC-31 — or whether origin independence is additionally required — remains a Round 38 design question.

**Factors that Round 38 must evaluate:**
1. **Delivery integrity:** If the election system delivers a false VO-3 to the voter AND stores a matching false evidence record, the voter's receipt confirms a fabricated vote. The delivery process must be constitutionally protected — the voter must receive their receipt through a channel the election system cannot intercept or substitute.
2. **Aggregate gap:** VO-3 provides per-vote authenticity; aggregate evidence authenticity (did the full set of votes produce the tally?) requires an aggregate reference that per-vote receipts cannot individually provide.
3. **EC-01 interaction:** ADR-5 must assess whether voter-held VO-3 as an authenticity reference is compatible with receipt-freeness requirements.

**Current status:** CANDIDATE ONLY. Not promoted to confirmed AC-31 reference standard. Origin independence and AC-31 sufficiency remain unresolved.
...
### G.5 OBS-ADR3-01: Independent Reference Standard Legitimacy Observation

> **Any independently governed reference standard requires its own constitutional legitimacy chain. The existence of an independent reference standard does not itself establish constitutional legitimacy for that reference standard. Reference independence and reference legitimacy are distinct constitutional questions.**

**Constitutional significance:**

AC-31 requires that the reference standard be constitutionally independent of the election system. Independence addresses the structural relationship between the election system and the reference standard (they are not under common control). Legitimacy addresses the constitutional grounding of the reference standard itself — by what constitutional authority does the reference standard exist? Who holds L-1 through L-5 for the reference standard?

These are distinct constitutional questions:
- **Reference independence:** Is the reference outside the election system's control? (AC-31)
- **Reference legitimacy:** Is the reference itself constitutionally grounded? (L-1/L-5 for the reference)

An independent reference that has no constitutional grounding is independent but not legitimate. A legitimate reference that is system-controlled is legitimate but not independent (and violates AC-31). Both properties are required.

**Inheritance:**
- **ADR-6 (Certification):** CertificationAuthority relies on the Authenticity Stratum reference. If the reference lacks constitutional legitimacy, the certification that depends on it inherits that legitimacy gap.
- **ADR-7 (GovernanceState):** The reference standard's constitutional grounding may create an additional ElectionConstitution dependency — if ElectionConstitution authorizes the reference, the chokepoint analysis from ADR-2 H.3.1 extends to include the reference standard.
- **Round 38+:** The implementation of the independent reference standard must specify its own L-1/L-5 legitimacy chain, or identify which existing constitutional instrument provides it.

**This is NOT a new D43 instance.** It is an inherited governance dependency that ADR-6 and ADR-7 must account for. The reference standard is an architectural element of the Authenticity Stratum, not a new constitutional authority function.
...
**Design note:** The Authenticity Stratum access pathway depends on how the independent reference standard is realized. If the reference is voter-held (VO-3), the access pathway involves voters presenting receipts — a fundamentally different design from a centrally accessible reference. This design belongs to ADR-4 (for scope-related access) and Round 38+ (for the Authenticity Stratum implementation).
...
**ADR-4 inherits from ADR-3:** Three-stratum model, Completeness Stratum as Gap A-3 architectural resolution, Link 1 + Link 2 of the dependency chain (Link 3 is ADR-4's to establish), ADR3-INV-01 (non-collapse invariant).
...
**Certification independent access requirement (R4 strengthening):**

> **CertificationAuthority may not certify Authenticity Stratum compliance without independent access to the same independent reference standard required by AC-31. Certification cannot constitutionally rely solely upon audit reports, audit conclusions, or audit assertions about authenticity. Certification must independently evaluate authenticity evidence using the constitutionally valid reference basis.**

**Constitutional rationale:** If certification relies on audit's report that evidence is authentic, rather than on independent verification against the independent reference, the trust chain collapses: audit becomes a proxy for certification. This recreates the self-authentication problem at the certification level — the certifier is verifying audit's assertion, not the evidence. AC-31's requirement for an independent reference standard must extend through certification: the certifier must access the same independent reference that the auditor used, and perform its own authenticity evaluation.

- D39 (Results/Tallying) dependency remains: certification of election results cannot be fully designed until D39 is resolved
- OBS-ADR3-01 (reference legitimacy): CertificationAuthority inherits the reference legitimacy dependency — if the reference standard lacks constitutional grounding, the certification that depends on it carries that legitimacy gap
...
- OBS-ADR3-01 (reference legitimacy): ADR-7 must account for the reference standard's own legitimacy chain in the ElectionConstitution governance design — if ElectionConstitution is the L-1 source for the reference, the chokepoint analysis must include the reference standard's dependence on ElectionConstitution
- ADR3-INV-01 (non-collapse invariant): ADR-7 must respect that Completeness, Presence, and Authenticity are constitutionally independent — GovernanceState design must not collapse these strata
...
| B (External Reference Holder) | Satisfied | Satisfied | Partial | Partial | Low | Low | Not selected |
...
| VO-3 (ReceiptHash) | Per-vote voter-held receipt | CANDIDATE ONLY — custody independence established; origin independence and AC-31 sufficiency unresolved | Confirmed candidate; Round 38 design question; ADR-5 must assess EC-01 compatibility |

---

## Section — ARB Decision Block

**[SUBMITTED FOR FINAL ARB APPROVAL — REVISED PER 37-03R]**

### Decisions Made in This ADR

1. **EH-01 → AC-31 (Architectural Constraint):** Constitutional (audit-grade) evidence authenticity verification requires an independent reference standard. Scoped to audit-grade verification; individual voter-facing verification deferred to ADR-5 / EC-01. AC-31 requires reference independence — NOT a specific organizational form, technical mechanism, or implementation.

2. **CPR-05 → Option D (Stratified Evidence Architecture):** Three strata — Completeness (Gap A-3 / constitutional scope), Presence (Audit Context existing model), Authenticity (AC-31 independent reference). Strata are constitutionally distinct; none may substitute for another (ADR3-INV-01).
...
5. **VO-3 (ReceiptHash) constitutional role:** CANDIDATE ONLY for per-vote authenticity reference. Custody independence after delivery is established. Origin independence and AC-31 sufficiency remain unresolved Round 38 design questions.
...
10. **ADR3-INV-01 (Non-Collapse Invariant):** Completeness, Presence, and Authenticity are constitutionally distinct. No future architecture may satisfy one through another.

11. **OBS-ADR3-01 (Reference Legitimacy):** Independent reference standard requires its own constitutional legitimacy chain. Reference independence ≠ reference legitimacy. Inherited by ADR-6 and ADR-7.

12. **Certification Independent Access (R4):** CertificationAuthority must independently access the AC-31 reference standard — cannot rely solely on audit reports or assertions.

---

### Revision Summary (37-03R)

| Revision | Description | Location | Status |
|----------|-------------|----------|--------|
| R1 | AC-31 realization discipline — independence ≠ external organization | C.4 (new section), D.2, H.2, throughout | Complete |
| R2 | Stronger VO-3 qualification — custody vs. origin independence | F.4 | Complete |
| R3 | Independent Reference Standard legitimacy observation — OBS-ADR3-01 | G.5 (new section) | Complete |
| R4 | Certification dependency strengthening — independent access required | I.3 | Complete |
| R5 | Three-Stratum non-collapse invariant — ADR3-INV-01 | E.2 (new subsection) | Complete |

---

### Architectural Quality Self-Review

| Check | Result |
|---|---|
| No constitutional decision changed | ✓ All decisions from original ADR-3 preserved |
| No new authority function introduced | ✓ |
| No new D43 instance introduced | ✓ OBS-ADR3-01 is observation, not new instance |
| No implementation technology selected | ✓ AC-31 realization discipline added (R1) |
| No realization bias introduced | ✓ External organization language neutralized |
| ADR-4 dependency chain remains valid | ✓ Link 3 unchanged; ADR3-INV-01 strengthens |
| ADR-5 dependency chain remains valid | ✓ CT-1 conditional unchanged |
| ADR-6 dependency chain strengthened | ✓ R4 adds independent access requirement; R3 adds legitimacy inheritance |
| ADR-7 inheritance remains intact | ✓ R3 and R5 add new inheritance items |
| All five revisions traceable and visible | ✓ This table + revision notes in header |

---

### Final Readiness Assessment

**ADR-3 → READY FOR FINAL ARB APPROVAL**

All five required revisions applied. No constitutional decisions changed. Document precision, implementation neutrality, and architectural drift resistance improved. ADR-4 through ADR-7 inheritance strengthened.
...
The custody vs. origin independence distinction (R2) is now explicit. The delivery independence argument is constitutionally significant but incomplete — origin independence remains unresolved. ADR-3 maintains CANDIDATE ONLY status. Does the ARB agree with this classification?
...
Full deferral maintained. ADR-3 establishes stratum existence and constitutional source. Content enumeration is ADR-4's function.
...
Both forms are constitutionally valid under AC-31 (as clarified by R1 — reference independence ≠ specific organizational form). This design question is carried to ADR-4, where the AuditScopeAuthority structure may inform which form is most appropriate.

---

*Round 37-03R — ADR-3: Evidence and Verifier Architecture — REVISED — Submitted for Final ARB Approval*
