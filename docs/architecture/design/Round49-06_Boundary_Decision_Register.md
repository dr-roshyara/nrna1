...
**Status:** 🗂️ BOUNDARY DECISION REGISTER — **decisions only.** Evidence in `49-04`; reasoning in `49-05`. The authoritative software-boundary output consumed by Migration (`49-03`) + Aggregate Discovery (Round 50).
**Date:** 2026-06-26
...
|---|-----------|----------|-----------|----------------|----------|-------|
| BDR-01 | **Evidence** | **Confirmed BC** | High | Yes | EV-010, EV-011 | immutable System of Record |
| BDR-02 | **Voting** | **Confirmed BC** | Med-High | Yes | EV-070, EV-071 | anonymous vote SoR; Results=projection; Active-Record→R50 |
| BDR-03 | **Appointment (Authority)** | **Confirmed BC** | Medium | Medium | EV-050, EV-051 | owns Mandate truth; prior reversed (EP-5) |
| BDR-04 | **Contestation** | **Confirmed BC — greenfield** | High | Yes | EV-090 | S-5 standing; closes the loop; build target |
| BDR-05 | **Adjudication** | **Confirmed BC (certified) — UNREALIZED**; existing jurisdiction-arbitration = separate concern | Medium | **No** | EV-001, EV-002, EV-003 | scope question open; election-determination behaviorally absent |
| BDR-06 | **Replay** | **Application Capability** (over Evidence) | Medium | **No** | EV-020, EV-021 | revisit after implementation (owns Session/Cert records) |
...
## Tally
**Confirmed BCs: 5** (Evidence, Voting, Appointment, Contestation, Adjudication[unrealized]). **Application Capability: 1** (Replay). **Supporting Subdomain/Service: 2** (Authorization, Lifecycle). **Infrastructure: 1** (Audit). **Read Models: 2** (Results, Legitimacy). **Invariant: 1** (Anonymity). **External: 1** (Trust-Anchor/Consent).
