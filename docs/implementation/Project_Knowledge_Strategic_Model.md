...
- **Theory 4 watch (ARB, 2026-07-11 — emerging, NOT designed):** the chain *Knowledge Need → Working Context → Context Evaluation → Gap Evidence → Knowledge Improvement* sketches a fourth theory — **Knowledge Evolution** ("how does knowledge improve over time?"). Deliberately unbuilt; the pilot watches for it. If it emerges from real work, it earned itself.
- **Validation horizon (ARB):** after ~20–30 real engineering tasks under this model, the evidence answers: is KnowledgeObligation needed? is KnowledgeNeed the right aggregate? is ContextEvaluation sufficient? does assembly remain minimal? do recurring patterns justify automation? **Only then does this architecture evolve.** RQ-002 program status: **COMPLETE — Constitution and Strategic Model frozen; the next source of change is operational evidence.**
