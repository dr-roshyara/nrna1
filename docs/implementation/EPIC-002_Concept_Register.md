...
| 2026-07-13 | Iteration 3a (`wf_3f9f7018-a37`, resumed as task `wg40ljxqs`, ElectionGuard/E2E-VV) | **INVALID — Research Infrastructure failure (NEW failure class: synthesis-stage corruption, not rate-limiting).** Extraction PASS, Verification PASS, Synthesis FAILED (placeholder stub returned). 7 refuted claims retained as legitimate content; confirmed findings UNKNOWN, not zero. **NO register update, no phenomenon-table update, no promotions** — per the Research Workflow Qualification rule, an invalid iteration touches nothing (Evidence Set — the 7 refuted claims — retained; Synthesis alone failed) | Excluded entirely from the stopping-criterion count (not a research result of any kind) |
...
|---|---|---:|---|---|
| Evidence is structurally plural / multi-kind | ES: statistical-verification splits (custody-vs-certification, auditability≠auditing) · AL/CL: judicial-review doctrine (standard-of-review taxonomy) · PR: ontology (Entity/Activity/Agent) · TE: safety-assurance argumentation (heterogeneous-artifact traceability) | **4** | too early — enters falsification phase as the strongest candidate | Repeated cross-disciplinary phenomenon → requires falsification, then synthesis |
| An adjudication/correction mechanism is neither automatic nor guaranteed reliable | ES: statistical verification (declare-failure) · AL: judicial-review doctrine (Bestandskraft, curing rules — gameable) · PR: ontology (adjudication left to "unspecified means") | **3** | too early | Repeated cross-disciplinary phenomenon → requires falsification, then synthesis |
| Contemporaneity — no retrospective reconstruction of record or justification | ES: statistical verification (commit-before-sample) · AL/CL: judicial-review doctrine (Chenery/hard-look) | **2** | too early | Repeated cross-disciplinary phenomenon → requires falsification, then synthesis |
| Evidence-substrate is separable from argument/trust-judgement over it | PR: ontology (scope boundary, medium confidence) · TE: safety-assurance argumentation (case-vs-evidence framing; most direct claims refuted) · DF: forensic procedure (hedged — NIST legal-sufficiency disclaimer is a scope DISCLAIMER, not a conceptual treatment) | **2 (+1 hedged)** | too early — confidence uneven across legs | Repeated cross-disciplinary phenomenon → requires falsification, then synthesis |
| Chain of custody / evidence integrity as a named practice | ES: statistical verification (compliance-audit custody, embedded) · DF: forensic procedure (NIST custody log, direct, 3-0) | **2** — CAUTION: both legs partly share the procedural-custody mechanism; family independence weaker than the count suggests (flagged per the ARB same-school warning) | too early — independence itself needs testing | Repeated cross-disciplinary phenomenon → requires falsification, then synthesis |
...
**Reading this table (bookkeeping, not synthesis):** five phenomena sit at ≥2 evidence families; one remains single-family. **Per the ARB's caution (2026-07-12): these counts are entry tickets to the falsification phase, not conclusions** — recurrence can reflect one school of thought echoing across fields (the chain-of-custody row is explicitly flagged for exactly this risk). The falsification phase now tests each phenomenon against the remaining disciplines: a phenomenon survives only if mature fields with different assumptions, methodologies, communities, and terminology fail to produce a counterexample.
