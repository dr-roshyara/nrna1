...
- **`FinalizeDeterminationCommand` — REJECTED today, with a paired reversal condition — and an honest finding, now put through the ARB's dormant-mechanism trichotomy.** `finalize()` is an implemented aggregate method with **no production caller and no command anywhere** (the EPIC-003 inventory found zero callers). Per the litmus, a method's existence cannot justify a command. **The trichotomy forced (per ARB — the methodology must make this question answerable, not decide it in advance):**
  ...
  **Recorded finding for the ARB:** the dormant mechanism is most plausibly a missing business intention whose definition is entangled with Q-2. **Refined per ARB review — the intention may not be a command at all:** if finality's trigger is the challenge window closing, then finalization is a **temporal business policy** (*Challenge Window Closed → policy → finalize*), and the intention belongs to the policy or Process Manager reacting to time — in which case `FinalizeDeterminationCommand` **may never exist**. An aggregate command and a time-reacting policy are different realizations of the same business concept; choosing between them now would presuppose Q-2's answer. **Paired reversal condition (corrected accordingly):** when a finality-observing business need emerges or Q-2 resolves, *the intention, the event candidate, and the realization question (command vs. temporal policy vs. PM reaction) return together* through ARB review — the reversal condition no longer presupposes the command shape. Flagged, not decided.
...
- Additional cluster-adjacent finding (this artifact): the dormant `finalize()` trichotomy leans toward a missing intention **entangled with Q-2**, not with the PM — a second, distinct gravitational center (finality/window), recorded separately so the two are not conflated. **Explicit cross-reference (per ARB):** the two centers may interact — if finalization proves to be a temporal policy, its *executor* could plausibly be the PM (timer → PM → finalize) even though its *definition* belongs to Q-2. Resolving either center should check the other; neither absorbs the other pre-emptively.
...
**Artifact №7 is accepted and FROZEN** with the DMT enrichment (finalize() → missing-intention hypothesis, Q-2-entangled, realization shape open: command vs. temporal policy vs. PM reaction) and the corrected paired reversal condition. The **Dormant Mechanism Trichotomy (DMT)** is adopted as the sixth permanent governance principle. Artifact №8 (Repositories) is authorized.
