...
- **`FinalizeDeterminationCommand` — REJECTED today, with a paired reversal condition — and an honest finding.** `finalize()` is an implemented aggregate method with **no production caller and no command anywhere** (the EPIC-003 inventory found zero callers) — a dormant mechanism awaiting an intention that no business process currently expresses. Per the litmus, a method's existence cannot justify a command. **Paired reversal condition:** this rejection travels with `DeterminationFinalized`'s (artifact №6) — if a finality-observing business process emerges, *the intention, the command, and the event return together* through ARB review, as one package. Until then, the dormant method is recorded honestly as implemented-ahead-of-intention.
...
- **The Emergent Design Cluster, now four members** (FailureDeclared · DeclareFailure · EvidenceSet · R-4-expanded seat) plus the loop-head request shape — the PM design's agenda continues to assemble itself.
