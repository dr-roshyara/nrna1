...
> **⚖️ ANNOTATION (2026-08-02 — added, not rewritten): §WP-4 IS SUBDIVIDED per R-68 and REMAINS OPEN.** **WP-4A — APM inbox wiring** (`(Adjudication, ChallengeRouted)` handler + consumer-side registration) is **ACCEPTED AND CLOSED by R-69**. **WP-4B — crash-safe conclude→issue seam · WP-4C — `AdjudicationFailureDeclared` + counterpart + hydrator + catalog entry · WP-4D — authority-decision intake port + interim adapter** are **OPEN**, each dependent on the authority-decision intake (a recorded external). **§WP-4 closes, and the Phase DoD's *“all eight WPs accepted”* is satisfied for this row, only when 4B, 4C and 4D are disposed of.** Method follows WP-3 → 3A/3B and WP-7 → 7A/7B/7C. See `engineering/verification/reports/2026-08-02-wp4-acceptance-review.md`.
