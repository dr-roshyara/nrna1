...
| ENG-004 | Mutation Ratchet 1 | Backlog | First deliberate MSI raise per the A-3 baseline-then-ratchet policy (PB-007 7C baseline: MSI 75% · Test Strength 96% · 22 escaped · 184 uncovered). Scope: extend coverage into the uncovered 22%, then hunt escapees — **first target `ChallengeReactionOutcomeTranslator`** (4 escaped; the business-condition→inbox-marker translation boundary in the constitutional correction loop — ARB's named highest-value mutation territory). Raise `min-msi` only at completion (never retroactive, never lowered). Opened from ARB 7C acceptance (2026-07-10). |
