...
| ENG-008 | **Externalize the link-repair confidence policy into configuration** | Backlog — **policy/execution separation** | `scripts/link-check.php` currently holds the confidence thresholds in code (`git-rename`/`documented-migration`/`exact-target` = 100 · `unique-basename` = 99 · `ambiguous` = 75 · `missing` = 0, auto-apply at ≥99). **Target shape: a `link-policy` configuration owned by governance, with the script reduced to executing it** — the same registry-and-resolver separation already applied to documentation placement. Opened by ARB direction, 2026-08-01: *"KnowledgeOS owns policy. Tool executes it. That separation ages much better."* Does NOT block anything. |
