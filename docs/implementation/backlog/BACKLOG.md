...
| ENG-005 | Stream B Documentation Consolidation | Proposed (pending P-6 ARB decision) | From retrospective recommendation P-6 (2026-07-11): apply the documentation rule (IDD = implementation decisions · ADR = architectural decisions · retrospective = lessons · CONTEXT = current state only) — refresh stale c4 · remove superseded progress tables · de-duplicate rulings across record types (evidence O-10). No redesign (dependency map §6 Stream B). |
