...
- **Honest reading (Interpreted):** Test Strength is 65%, not 96% — the "assertions are strong, only coverage is missing" story was a measurement artifact. Escapes concentrate exactly where the ARB named the highest-value mutation territory: the messaging/persistence boundary of the constitutional correction loop. ENG-004 (Mutation Ratchet 1) baselines against THESE validated numbers.
