...
- **DD-4 — electionId resolution — DOMAIN INVESTIGATION (ARB-directed; resolved domain-inward, not transport-outward).**
  ...
  - **DD-4a — model `TargetRef` as a domain concept (ARB-directed; BEFORE any ADR).** `TargetRef:string` is **Primitive Obsession** hiding a business concept. Modeling:
    ...
    - **Emergent Value Object:** `TargetReference { ElectionId electionId · TargetType type · TargetId targetId }` replaces `TargetRef:string`. `electionId` becomes part of the **ubiquitous language**, not something Election "requested." Anonymity preserved: `type`/`targetId` are outcome/determination references only (the constitutional guard, now scanning Contestation, forbids voter tokens).
  - **Resolution (domain model first, then contracts):** evolve the **published language around `TargetReference`** — `Challenge` targets a `TargetReference`; `ChallengeRaised` / `Determination` / `DeterminationIssued` carry `TargetReference` (which *contains* `electionId`). Election reads `electionId` from `TargetReference` **because that is the ubiquitous language**. Read-model lookup is the fallback only if the VO model is rejected.
  - **Anonymity:** safe — `TargetReference` is **not** voter↔vote linkage (`VoteAccepted` already carries `electionId`; ADR-T11 forbids linking a voter to a vote, not election/target identity).
  - **Impact / dependency:** a **Strategic DDD refinement + Published Language Evolution** touching **Contestation + Adjudication** (already-built greenfield contexts): `TargetRef:string → TargetReference` VO, propagated through `Determination`, then `DeterminationIssued → v2` (ADR-T5; v1 retained). Requires **ARB approval**, sequenced **before** PB-004. **ARR re: the frozen Messaging Platform is unaffected — consume-only.**
  - **Requested ARB ruling (revised — no ADR yet):** approve the **`TargetReference` Value Object model** (and the *Challenge→one Election* invariant it makes explicit). **Then** the ADR is written as a **Published Language Evolution** (why the UL changed: `TargetRef` primitive → `TargetReference` VO), **then** `DeterminationIssued v2` around the VO, **then** PB-004 RED. **PB-004 RED waits on this prerequisite.**
...
---
**Awaiting ARB:** **ruling on DD-4a** — approve the **`TargetReference` Value Object** model (ElectionId · TargetType · TargetId) that replaces the `TargetRef` primitive, **before** any ADR. On approval the sequence is: (1) Strategic DDD refinement `TargetRef → TargetReference` in Contestation; (2) ADR describing the **Published Language Evolution**; (3) `DeterminationIssued v2` around the VO (ADR-T5, v1 retained); (4) PB-004 RED. **PB-004 RED is blocked on this prerequisite** (a cross-context Contestation+Adjudication change), NOT on Messaging (consume-only). No ADR and no code until the VO model is approved.
