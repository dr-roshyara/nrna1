...
- **DD-4 — electionId resolution — DOMAIN INVESTIGATION (ARB-directed; resolved domain-inward, not transport-outward).**
  ...
  - **DD-4a — model `TargetRef` as a domain concept (ARB-directed; BEFORE any ADR).** `TargetRef:string` is **Primitive Obsession** hiding a business concept. Modeling:
    - *What can be challenged?* (evidence: `TargetRef` doc + Round50) — an **Election Outcome** (a result/outcome) or a **prior Determination**. Never an individual vote (`TargetRef` "carries no vote content" — anonymity).
    - *Does each target type belong to exactly one Election?* **Yes** — an Election Outcome → one Election; a Determination is issued within one Election's context → one Election.
    - *Identifying attributes (domain, not transport):* `ElectionId` · `TargetType` (ElectionOutcome | Determination | …future) · `TargetId` (the outcome/determination id — **never** a vote/voter id).
  - **DD-4b — meaning BEFORE reference (ARB-directed final UL exercise).** Ask *"what is being contested?"* before *"how do we reference it?"*
    - *What can legally be challenged?* (evidence, ER-02 — only what is modeled today) — an **Election Result** (outcome) or a **prior Determination**. No other target types are modeled; do not invent (nominations/membership/audit are not in scope without evidence). `SubmittedContent` is just the challenger's `claim`.
    - *What unifies them?* Both are **an authoritative outcome of the constitutional process that a standing-holder may legally contest within the appeal window.**
    - **Naming review (ARB-directed; business term beats software term):** the certified vocabulary **already has the term** — BDR v1.1 (Round49-06, *immutable*): *"adjudicate a **contested outcome** → binding determination"*; Round50: *"the **contested outcome**."* Per ER-02 + "prefer the constitutional term over an invented concept," the concept is **`ContestedOutcome`** — anchored in frozen certified language, **not** the invented `ContestableDecision`. (A prior Determination challenged is treated as a contested outcome of a prior adjudication — the BDR term generalizes.) → recommend ratifying `ContestedOutcome`.
    - *Then the reference emerges:* `Challenge` does not hold a primitive; it holds a **`TargetReference`** value object that **identifies a `ContestedOutcome`** — `{ ElectionId electionId · TargetType type(ElectionResult | Determination | …future) · TargetId id }`. The concept (`ContestedOutcome`) is the meaning; `TargetReference` is the supporting VO that points at it. `electionId` is part of the concept's identity, not something Election "requested."
  - **Glossary (ubiquitous language — before any ADR):**
    - **Challenge** — a standing-holder's formal, time-bounded contest of a `ContestedOutcome`.
    - **ContestedOutcome** — *(certified term, BDR v1.1)* an authoritative outcome of the constitutional process that may be legally challenged; today an **Election Result** or a **prior Determination** (extensible via `TargetType`).
    ...
    - **TargetReference** — the VO identifying the `ContestedOutcome` a Challenge contests: `ElectionId · TargetType · TargetId`; reference only, no vote content (anonymity).
  ...
  - **Resolution (concept → VO → published language → contract):** model `ContestedOutcome` + its `TargetReference` VO in Contestation; `Challenge`/`ChallengeRaised`/`Determination`/`DeterminationIssued` carry `TargetReference`. Election reads `electionId` from it **because that is the ubiquitous language**. Read-model lookup is the fallback only if the concept is rejected.
  - **Anonymity:** safe — `TargetReference` is **not** voter↔vote linkage (`type`/`id` are Result/Determination refs; `VoteAccepted` already carries `electionId`; ADR-T11 forbids linking a voter to a vote, not decision/election identity).
  - **Scope / governance (ARB ruling — NOT a PB number):** this is a **cross-context evolution**, not `PB-003.5` and not part of PB-004. It is governed as: **ADR (Published Language Evolution) → Strategic DDD refinement (`TargetRef`→`ContestedOutcome`+`TargetReference` in Contestation) → `DeterminationIssued v2` (ADR-T5, v1 retained) across Contestation+Adjudication → then PB-004**. Messaging unaffected (consume-only).
  - **Requested ARB ruling (no ADR yet):** **ratify** (1) the **`ContestedOutcome`** concept name — evidence-anchored in BDR v1.1's certified "contested outcome" (not invented); (2) the **`TargetReference`** supporting VO (`ElectionId · TargetType · TargetId`); (3) the **glossary** as Contestation's ubiquitous language. **Then**, per **ER-06** (UL before Published Language): a tiny **ADR — Ubiquitous Language Evolution** (`TargetRef` → `ContestedOutcome` + `TargetReference`) → the **Published Language Evolution ADR** referencing it → `DeterminationIssued v2` (ADR-T5) → PB-004 RED. This concept/glossary **graduates out of this IDD into the UL-Evolution ADR** once ratified (the IDD then references it — the work is Strategic DDD, not PB-004). **PB-004 RED waits on this prerequisite.**
...
---
**Awaiting ARB:** **ratify DD-4b naming** — concept **`ContestedOutcome`** (evidence-anchored in BDR v1.1's certified "contested outcome", not invented) + `TargetReference` VO (`ElectionId · TargetType · TargetId`) + glossary as Contestation's ubiquitous language. On ratification, per **ER-06**: tiny **ADR — Ubiquitous Language Evolution** → **Published Language Evolution ADR** → `DeterminationIssued v2` (ADR-T5) across Contestation+Adjudication → **then PB-004 RED**. Concept/glossary graduates into the UL-Evolution ADR (Strategic DDD, not PB-004). Blocked on this prerequisite; Messaging unaffected (consume-only). No ADR, no code until the name is ratified.
