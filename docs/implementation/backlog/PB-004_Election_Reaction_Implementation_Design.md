...
- **DD-4 — electionId resolution — DOMAIN INVESTIGATION (ARB-directed; resolved domain-inward, not transport-outward).**
  ...
  - **DD-4b — meaning BEFORE reference (ARB-directed final UL exercise).** Ask *"what is being contested?"* before *"how do we reference it?"*
    ...
    - *What unifies them?* Both are **authoritative decisions produced by the constitutional process that a standing-holder may legally contest within the appeal window.** → the domain concept is **`ContestableDecision`** (the *meaning*). *(Alternative name `ContestableOutcome` — ARB to ratify the term; "decision" is chosen because a Determination is a ruling and a certified Result is the electorate's decision.)*
    - *Then the reference emerges:* `Challenge` does not hold a primitive; it holds a **`TargetReference`** value object that **identifies a `ContestableDecision`** — `{ ElectionId electionId · DecisionType type(ElectionResult | Determination | …future) · DecisionId id }`. The concept (`ContestableDecision`) is the meaning; `TargetReference` is the supporting VO that points at it. `electionId` is part of the concept's identity, not something Election "requested."
  - **Glossary (ubiquitous language — before any ADR):**
    - **Challenge** — a standing-holder's formal, time-bounded contest of a `ContestableDecision`.
    - **ContestableDecision** — an authoritative decision of the constitutional process that may be legally challenged; today an **Election Result** or a **prior Determination** (extensible via `DecisionType`).
    - **Election Result** — the certified outcome of an election (a `ContestableDecision` owned by Voting/Election).
    - **Determination** — a binding ruling issued by Adjudication on a Challenge (itself a `ContestableDecision` when challenged).
    - **TargetReference** — the VO identifying the `ContestableDecision` a Challenge contests: `ElectionId · DecisionType · DecisionId`; reference only, no vote content (anonymity).
    - **ElectionId** — identity of the single Election a `ContestableDecision` belongs to.
    - **DecisionType** — which kind of `ContestableDecision` (`ElectionResult | Determination | …future`).
  - **Resolution (concept → VO → published language → contract):** model `ContestableDecision` + its `TargetReference` VO in Contestation; `Challenge`/`ChallengeRaised`/`Determination`/`DeterminationIssued` carry `TargetReference`. Election reads `electionId` from it **because that is the ubiquitous language**. Read-model lookup is the fallback only if the concept is rejected.
  ...
  - **Scope / governance (ARB ruling — NOT a PB number):** this is a **cross-context evolution**, not `PB-003.5` and not part of PB-004. It is governed as: **ADR (Published Language Evolution) → Strategic DDD refinement (`TargetRef`→`ContestableDecision`+`TargetReference` in Contestation) → `DeterminationIssued v2` (ADR-T5, v1 retained) across Contestation+Adjudication → then PB-004**. Messaging unaffected (consume-only).
  - **Requested ARB ruling (no ADR yet):** approve (1) the **`ContestableDecision`** domain concept (and term), (2) the **`TargetReference`** supporting VO, (3) the **glossary** as Contestation's ubiquitous language. **Then**: ADR (Published Language Evolution) → domain refinement → `DeterminationIssued v2` → PB-004 RED. **PB-004 RED waits on this prerequisite.**
...
---
**Awaiting ARB:** **ruling on DD-4b** — approve (1) the **`ContestableDecision`** domain concept (meaning-first) + term, (2) the **`TargetReference`** supporting VO, (3) the **glossary** as Contestation's ubiquitous language — **before** any ADR. On approval, governed as a **cross-context evolution (NOT a PB number)**: ADR (Published Language Evolution) → domain refinement (`TargetRef`→`ContestableDecision`+`TargetReference`) → `DeterminationIssued v2` (ADR-T5) across Contestation+Adjudication → **then PB-004 RED**. Blocked on this prerequisite; Messaging unaffected (consume-only). No ADR and no code until the concept + VO + glossary are approved.
