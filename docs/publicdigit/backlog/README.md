...
| [`PBDIGIT-48`](PBDIGIT-48-election-state-has-four-representations.md) | **Election state has four representations and they disagree** — projection `voting_active` vs `state` vs `status` vs `is_active` | `EPIC-03` | `OPEN` — the cause behind `PBDIGIT-47` |
