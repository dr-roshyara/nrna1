...
| [`PBDIGIT-68`](PBDIGIT-68-election-membership-is-a-durable-entitlement.md) | ✅ **RULING: `ElectionMembership` is a durable election entitlement** (Model B, PO 2026-08-12) — closes Appendix E's `NOT SPECIFIED`. 🔴 But **Chief suspension may not block voting**, and `admit`/`revoke`/`restore` have no constitutional names | `EPIC-03` | **DECISION RECORDED** · consequences `G-1`…`G-7` open; Constitution **not** amended |
