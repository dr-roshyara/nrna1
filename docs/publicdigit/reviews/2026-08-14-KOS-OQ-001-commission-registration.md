...
**Two-stage clause honored:** implementation begins only now that (a) the grant is registered AND (b) the machine state confirms S3's authorization — verified below by the mechanism's own `authorized` query (R6), not by prose.
