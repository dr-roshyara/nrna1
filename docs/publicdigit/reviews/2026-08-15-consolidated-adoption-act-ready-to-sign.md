# Consolidated Adoption Act — ready to sign

**Prepared by Governance (Session 2) · 2026-08-15 · One act, five items**
**⛔ NOT adopted. This is the text of an act for the PO/ARB to perform. Governance does not infer an adoption from a recommendation.**

---

## Already done by Governance under express delegation *(not part of the act)*

**`EM-OPEN-044` — RULED. The canonical term is *"progression is HALTED"*; *"stopped"* is retired, and `EM-GOV-011`'s wording is aligned. Meaning unchanged; no rule re-decided.** *(Delegation: "Governance should make the final wording choice.")*
...
>
> **5 · `EM-OPEN-050`(b)** — *The policy version and duration applicable when a recovery period begins remain bound to that recovery period. A later service-policy change does not retroactively alter an existing election recovery deadline, and may not shorten a deadline already communicated.*
>
> **— Signed: PO/ARB, 2026-08-15**

## What this act does NOT do — stated so the boundary is visible in the act itself

* **`EM-GOV-013`** *(rescheduling recovery)* — **not included; it stays blocked** until `EM-GOV-004`'s six boundaries are decided, because rescheduling an already-elapsed window is one of them.
...
**Adopted:** `EM-GOV-012` · `EM-GOV-014` (both parts) · `EM-OPEN-047` · `EM-OPEN-050`(b) · *(and `EM-OPEN-044` already ruled)*.
**Still pending/blocked:** `EM-GOV-013`. **Architecture ⏸️ · Session 3 🛑 · Session 1 must not verify unauthorized work.**

**Traceability.** `EM-GOV-004`…`011` · `EM-VOC-004`/`005` · `EM-VOT-004`/`005` · `P-2H` · `D-2` · `F-PROTO-1` · `EM-OPEN-039`/`042`/`044`…`050`.
