...
4. **NULL/ABSENT TENANT CONTEXT = OPEN SEMANTIC QUESTION (PA-confirmed, 2026-08-14).** The table does not specify it, and the three candidate answers are **materially different** in security and architecture: **(1)** no context → deny · **(2)** no context → derive the context from the election · **(3)** no context → platform/default organisation. The current code's platform-org fallback is **implementation evidence, not business authority**. ⛔ **Session 3 must not choose a behaviour merely from current implementation** — the answer is one PO line, naturally at boundary presentation.
5. **PA confirmation of the P6 re-reading (2026-08-14), with the precise formulation now on record:** *"The system correctly denies voting in a mismatching tenant, but it does not reliably distinguish that legitimate denial from a denial produced by an incorrect tenant context and subsequently cached."* Also confirmed: the cache observation stays an observation — the grant must not pre-decide *"change the cache key in `User.php`"*; cache identity is Session 3's design question after the grant, under the invariant that a result produced under a different organisational context is never reused.
