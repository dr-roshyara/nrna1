...
| **`EM-GOV-012`** | 🟡 **PENDING — one line adopts it.** **"At the moment a mandatory condition prevents the current phase from completing, the election's progression is HALTED and the event and reason are recorded. No downstream phase occurs. The election is not thereby cancelled, completed, or suspended. Its subsequent recovery or termination is a separate governed decision."** **Reason recorded with it:** *halted* **describes what happened**; *suspended* **names a particular governance mechanism for handling it** — and the mechanism is not yet chosen. **Generalisable principle: describe the fact before naming the remedy.** | 🟡 **PENDING PO ADOPTION** *(offered by the PO as a recommendation — *"my recommendation"*, *"I would give"* — not yet a performative act; Governance does not convert a draft into a rule)* |
| **`EM-GOV-013`** | 🟡 **PENDING — one line adopts it.** **"When a phase progression is halted, the election cannot continue using the existing schedule. The affected phase must be rescheduled through a governed action. Any subsequent phases whose planned schedule depends on the halted phase must be reviewed and, if necessary, rescheduled. All changes, decisions, reasons and new schedules must be recorded in the election protocol. After rescheduling, the election continues only when the normal Election Rules are satisfied again."** **Model: halted → protocol → reschedule → re-evaluate → continue. Suspension becomes an EXCEPTIONAL mechanism, not the primary recovery path.** ⚠️ **A reschedule is not a correction of history: the old schedule is superseded and preserved, never silently changed.** | 🟡 **PENDING PO ADOPTION** *(offered as *"I would formulate"* / *"the core rule can be"* — a draft, not a performative act)* ⛔ **AND BLOCKED — see `EM-GOV-004`** |
| **`EM-GOV-014`** *(SPLIT 2026-08-15)* | 🟡 **PENDING — one line adopts it. Two parts, deliberately separated.** **PART 1 — Election domain (this Manifesto):** *"When election progression is halted, the election requires a governed recovery action; it cannot continue without one. The Election Chief is responsible for initiating it. The organisation may replace the Election Chief according to its governance rules; a replacement Chief inherits responsibility for the recovery decision but cannot bypass any Election Rule or reuse invalidated decisions. **Replacing the Chief does NOT reset history — the protocol remains the source of truth.** A recovery deadline **exists**. Reaching it is a recorded event; **what that event means is a governed decision, not an automatic consequence.**"* **PART 2 — NOT an Election Rule:** *"The recovery period **duration** is a service-policy parameter owned by the service provider and may change under service-policy governance. The Election Rules do not define the numeric duration. The system must record the applicable recovery-policy version and the deadline used for each halted election."* | 🟡 **PENDING PO ADOPTION** *(a formulation, not a performative act)* ⚠️ **AND: Part 2 moves a parameter OUTSIDE the Election context — see `EM-OPEN-050`** |
...
| **EM-OPEN-047** *(PO direction received; ownership question raised, 2026-08-15)* | 🔴 **PO direction: *"cancellation based on service provider policy."*** ⚠️ **Governance must flag a tension before recording it, because it appears to reverse the sentence agreed one step earlier — *"reaching it is a recorded event; what that event means is a governed decision, not an automatic consequence."*** **The distinction that matters: owning the DURATION is not the same as owning the CONSEQUENCE.** A duration is an operational parameter; **terminating an election is a business outcome.** **Three readings — which is meant?** **(i)** the **rule** *expiry ⇒ cancellation* is an **Election Rule**; only the **duration** is service policy *(what the split said)* · **(ii)** **both** duration and consequence are service policy — a provider could configure *expiry ⇒ cancel* in one deployment and *expiry ⇒ escalate* in another, **so the same failure would end differently for different customers** · **(iii)** cancellation is a **service act** in its own right *(an abandoned election closed for service reasons, akin to non-payment)* — legitimate, **but a SERVICE act, not an election-governance act.** ⚠️ **Compare `EM-GOV-006` (ADOPTED): a governed period exists only where an **Election Rule** deliberately establishes it — no scheduler behaviour, timeout or implementation convenience constitutes one. Reading (ii) would let a service parameter do what that rule denies a timeout.** ✅ **DERIVED and binding under EVERY reading (`EM-GOV-005`): the protocol must record WHAT caused the cancellation — a provider-caused cancellation must never be recorded as though the Chief or the Election Rules caused it.** | election recovery · ownership |
...
| **EM-OPEN-050** | ⚠️ 🔴 **The recovery-period split moves ownership of a parameter OUTSIDE the Election context — the first time any rule in this Manifesto does so.** **(a)** Under the standing Strategic-DDD obligation this needs its answers **before** it is built on: **which context owns the recovery-period policy · is this a Published-Language interaction · does the context map change?** **Governance records the business rule; the context-map consequence is Architecture's, and Architecture is PAUSED.** · **(b) ⚠️ RETROACTIVITY: if the provider changes the policy from 30 days to 14 while an election is already halted, does the new value apply?** **The anti-rewriting rules (`EM-GOV-005`, `EM-VOC-005`, `D-2`) exist to prevent exactly this class of change** — a later fact silently altering an earlier commitment. **Governance does not rule it, because the policy sits in another context**, but flags that *"record the policy version and deadline used"* **implies the answer without stating it.** · **(c)** may a service-policy change ever **shorten** a deadline already communicated? | context boundary · service policy |
...
| **EM-OPEN-044** | ⚠️ 🔴 **FOURTH vocabulary risk — and the opposite kind: TWO WORDS FOR ONE CONCEPT.** `EM-GOV-011` (ADOPTED) says **"progression STOPS at that phase"**; the pending `EM-GOV-012` says the election's progression is **"HALTED"**. **If both stand, every future reader must ask whether *stopped* and *halted* differ — and they do not.** **Two words for one concept damages a rule set as surely as one word for two concepts** *(the `eligible` and `outcome` defects were the latter kind)*. ⛔ **Pick one before `EM-GOV-012` is adopted, and align the other rule's wording in the same act.** *Governance expresses no preference between the two words.* | vocabulary integrity |
