...
| **`EM-GOV-006`** | **A post-schedule decision period exists only where an Election Rule deliberately establishes it. No technical behaviour constitutes one** — not a cron/scheduler grace period, hard-coded allowance, retry window, delayed job, clock-drift allowance, interface grace, automatic transition grace, or any implicit *"a few minutes late is fine"*. | 🟡 **PROPOSED** |
...
| 8 | any post-schedule period must be explicitly governed; **no technical grace period constitutes one** | ⚠️ **NOT ADOPTED — this is `EM-GOV-006`, PROPOSED above.** The instruction lists it among principles *"already established"*; **on the record it is not.** A statement that something is already established does not establish it. **One line adopts it; until then it binds nothing.** |
