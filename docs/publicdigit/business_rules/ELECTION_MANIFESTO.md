...
| **EM-OPEN-040** | 🔴 **What is the business outcome of a scheduled phase opportunity that NEVER became progression-permitted, because its predecessor never legitimately completed?** *(Decision request: `2026-08-15-EM-OPEN-040-decision-request.md`.)* **Confirmed: none of the five fits** — *started* no · *temporarily unable* is a **condition, not an ending** · *expired **unused*** implies it **was usable** · *cancelled* implies an act nobody performed · *superseded* only if a correction actually happens. **Options: ① a distinct sixth outcome · ② rule that one of the five covers it, with the distinction carried by the protocol · ③ it ends only when something ends it — with the cost that if nobody acts it NEVER ends · ④ another rule.** ⚠️ **Two dependencies raised: (i) *WHEN* it ends depends on `EM-OPEN-026`(a)/`028` — Governance separates *which outcome* (answerable now) from *when it is reached* (not) · (ii) upstream failure and downstream unreachability are DIFFERENT events — the five outcomes plausibly already describe the phase that failed its OWN conditions; the gap is specific to the phases downstream of it.** **DERIVED and binding on any answer: it must stay distinguishable, be recorded with its preceding refusals, and never silently revive.** **Governance holds no position.** | voting-opportunity outcomes |
| **EM-OPEN-042** | 🔴 **Does the ELECTION itself have a terminal business outcome when it cannot proceed?** One failure makes **every** later phase unreachable — candidacy, voting, counting and result publication all share the fate. **No adopted rule gives an election a terminal position of its own.** *(Raised by `EM-OPEN-040`; not part of it, not decided.)* | election lifecycle semantics |
