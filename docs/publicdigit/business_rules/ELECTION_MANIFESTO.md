...
| **EM-OPEN-051** | 🔴 **Does an exceptional Service-Provider extension create a NEW voting opportunity, or continue the existing one?** ⚠️ **Naming it an *"exceptional extension"* rather than a *"correction"* does NOT by itself escape `EM-VOC-005`** — that rule (ADOPTED) attaches to **any correction to a published voting schedule**, and an extension changes the published schedule. **A definitional relabel is not a carve-out; only an express exception is.** ✅ **DERIVED ARGUMENT FOR CONTINUITY, and it is decisive rather than preferential:** under `EM-VOT-005` authorization is **opportunity-bound**, so **a new opportunity would require a NEW Chief authorization mid-vote — which nobody has given — and voting would have to stop at the very moment the extension was meant to let it continue.** **The new-opportunity reading is therefore operationally incoherent, not merely undesirable.** ⛔ **But narrowing an adopted rule is the PO's act, not Governance's.** **Draft sentence for the PO:** *"Notwithstanding `EM-VOC-005`, an exceptional extension approved after voting has started continues the existing voting opportunity and does not supersede it. Votes already cast remain valid within it."* | voting opportunity × exceptional extension |
