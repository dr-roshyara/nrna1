...
| **EM-OPEN-054** | 🔴 **What is the SCOPE of an acceptance — the whole election, a single post/contest, or a combination?** ⚠️ **Third option recorded and it may well be the answer: election-wide PROCEDURAL acceptance before counting, then post-specific RESULT acceptance before publication.** ✅ **Governance's supporting observation: the two gates accept DIFFERENT OBJECTS, so a uniform scope answer may be wrong.** Gate 1 accepts *the completed voting **process***, naturally **election-wide**; Gate 2 accepts *the counting and its **tabulation***, and **tabulations are per post**. **A single scope rule imposed on both would force one into an unnatural shape.** ⚠️ **ORDERING (PO, 2026-08-16): answered BEFORE electoral standing** — *"otherwise we don't know what decision these representatives are actually accepting."* **Governance accepts the correction; it had named standing as next, and scope constrains standing rather than the reverse.** ⚠️ **`Q3` SHARPENED (PO, 2026-08-16) into three:** **A** does acceptance establish *"the counting procedure was conducted according to the predefined rules"*? · **B** does it additionally establish *"the resulting tabulation is accepted as the election result"*? · **C** if both, is it **one combined decision or two sequential ones**? **Neither implies the other unless Governance says so.** ✅ **DERIVED either way (`EM-GOV-005` + the commission's own protocol list): a COMBINED decision must still record WHICH GROUND failed, or the record cannot say why acceptance failed.** ⚠️ **Observation, not a recommendation: the two refusals are not symmetric — accepting the process but not the figures is a recount situation; accepting the figures while rejecting the process accepts the output of a procedure found defective.** ⛔ **STANDING GUARD registered (PO): *do not introduce post-scoped acceptance unless we are deliberately prepared to define post-scoped progression* — so a small acceptance rule cannot silently redesign the lifecycle.** ⚠️ **`EM-OPEN-057` is PRIOR to the post-specific half of this question and may dissolve it.** | acceptance scope |
| **EM-OPEN-055** | 🔴 **Which persons or bodies have the governed right to participate in an acceptance decision — and from what source does that right come?** ⚠️ **Framed broadly on purpose (PO, 2026-08-16): the question is NOT *"define an Election Commission."*** One may eventually be the answer, **but Governance must arrive at it rather than be forced into it by the shape of the question.** **Sub-question: what does a representative REPRESENT?** *(candidate · party · electoral constituency · another governed stakeholder class — none endorsed.)* ⚠️ **`C-1`, to be answered knowingly: is the Election Chief an operational authority · an acceptance participant · both · or a NON-VOTING participant?** *(the fourth option is genuinely distinct)* — **a Chief who votes in the gate partly decides whether their own request is permitted.** **Blocked behind `EM-OPEN-054`.** ⛔ **Note: the `Election Commission`, `Acceptance Representative` and `Party` concepts do not exist in the adopted record; this question is where they would come from, if at all.** | electoral standing |
...
| **EM-OPEN-057** | 🔴 **What is the business UNIT OF THE RESULT?** *(one election result · several post results · **one election result CONTAINING post results**)* ⚠️ **This is PRIOR to the post-specific half of `EM-OPEN-054`, and it may dissolve it.** **If the business result is one election-level result containing post-level data, Gate 2 can remain ELECTION-WIDE even though the tabulation is post-structured — and the lifecycle need not become post-scoped at all.** ✅ **Governance searched: no adopted rule defines what a result is composed of.** The record has *publish results* as an act (`ElectionConstitution`), `results_published` as a state, and `EM-OPEN-013` on unpublishing — **but nothing says whether an election has one result or several.** | election result |
| **EM-OPEN-058** | 🔴 **Does any unresolved acceptance prevent result publication for the ENTIRE election, or may some results be published independently?** ⚠️ **Registered as a rule in its own right at PO direction — *"a major business rule"*, not a by-product of the scope answer.** ✅ **Decision-relevant fact, stated as a fact and not a recommendation: the two options are ASYMMETRIC IN REVERSIBILITY.** Withholding publication is fully recoverable — the results can still be published later. **Publishing is not: `PBDIGIT-60` makes publication an immutable constitutional fact with visibility as a separate control, never "unpublish."** **The PO weighs that; Governance only records it.** ⛔ **Must not be chosen merely because post-specific results are technically producible.** | result publication |
