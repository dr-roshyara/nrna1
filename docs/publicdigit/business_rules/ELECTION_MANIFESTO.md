...
| **EM-OPEN-037** | 🔴 **Two consequences of the `EM-OPEN-032` model that it did not state, and that must be adopted knowingly rather than inherited.** **(a) ⚠️ It generalises "opportunity" beyond voting.** `EM-VOC-004` is **voting-scoped by its adopted text** *(a published **voting** schedule defines a **voting** opportunity)*. Speaking of an *administration · candidacy · counting · result-publication* opportunity extends it — and **an opportunity is a context awaiting an authorized progression request, so the extension implicitly answers `G-PROG-1` and `EM-OPEN-030` for every phase: it makes ALL phases request-progressed.** That may be exactly the intent; **it must not arrive by inheritance.** · **(b)** does the single publication act bring **all five** opportunities into existence **simultaneously**, each then awaiting its own scheduled time and its own request? *(The model implies yes; no rule says it.)* | opportunity scope × `G-PROG-1` |
