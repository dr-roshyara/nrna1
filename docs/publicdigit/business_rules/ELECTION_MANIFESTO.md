...
| **EM-OPEN-042** *(REFRAMED 2026-08-15)* | 🔴 **When a mandatory condition prevents an election from progressing, does the election enter a governed SUSPENSION, and what are the rules for resolving and resuming it?** *(Decision request: `2026-08-15-EM-OPEN-042-decision-request-election-suspension.md`.)* ⚠️ **BLOCKER FIRST — THIRD VOCABULARY OVERLOAD, and the worst of the three: *suspension* is ALREADY an adopted concept meaning suspension of an ELECTION MEMBER** (`EM-GOV-002`/`003`, `EM-EO-004`, `EM-FM-003`/`004`, and open items `002`/`003`/`006`/`007`/`009`). **Under a shared word, `EM-GOV-002` — "the Chief may independently suspend" — would read as authorizing the Chief to suspend the ELECTION**, which nobody has decided and which Q3/Q4 pose as open. ⛔ **Name the two apart BEFORE answering anything else** *(candidate, not adopted: **member suspension** vs **election suspension**)*. **ALREADY DERIVED, no ruling needed:** **Q9 = NO** — suspension can never bypass an Election Rule (`EM-GOV-008` + `EM-VOC-005`/`EM-VOT-005`: *fail → suspend → unsuspend → proceed on the old authorization* is already impossible) · **Q8 = YES** — every suspension, reason, corrective action, removal and rescheduling event is recorded (`EM-GOV-005` + its negative half) · **Q6 half-derived** — a changed schedule always forces a NEW opportunity; resumption onto an UNCHANGED schedule depends on `EM-OPEN-026`(a)/`028`. **OPEN: Q1** *(does failure suspend the whole election — and is suspension AUTOMATIC or a governed human act? both coherent, different systems)* · **Q2** meaning · **Q3** who may suspend · **Q4** who may remove *(must it be the same authority?)* · **Q5** preconditions for removal · **Q7** may the schedule change while suspended *(depends on `EM-GOV-004`'s unadopted boundaries)*. **Compatibility checked: `EM-GOV-011` says progression *stops*, NOT that the election is suspended — the bridge is Q1, not an assumption; and suspension attaches to the ELECTION, not per-phase, so it does not collide with "no artificial outcome".** **`EM-OPEN-035` and Q3/Q4 are one authority model, not three.** **`EM-OPEN-009` travels with this.** | election lifecycle semantics |
