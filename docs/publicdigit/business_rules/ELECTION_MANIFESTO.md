...
**`EM-GOV-004` boundaries are deliberately NOT adopted and require separate business decisions:** moving a window forward · extending a window · correcting a window already elapsed · moving a window backward · the effect on voting credentials already issued · permissibility once votes have been cast · ⚠️ **SEVENTH boundary added 2026-08-15: may a COMPLETED phase's schedule be changed?** *(New — it is broader than "after votes have been cast": a legitimately completed administration phase involves no votes at all, yet changing its recorded window would alter a historical election fact.)* **Governance notes this one may already be answered by `EM-GOV-005`/`D-2` — a completed phase's schedule is a recorded fact — but records it as a boundary question rather than assuming the derivation covers a schedule change nobody has ruled on.** **An implementation that assumed any of these would be inventing policy.** ⛔ **BLOCKER RAISED 2026-08-15: the proposed NORMAL recovery path runs straight through these unadopted boundaries.** `EM-GOV-013` makes **rescheduling the ordinary response to a halted phase** — and rescheduling a phase whose window has **already elapsed** is precisely *"correcting a window already elapsed"*, which this rule **expressly does NOT adopt**; a cascade reschedule also moves windows **forward** and may **extend** them. **These boundaries were a later question while recovery was undefined. If `EM-GOV-013` is adopted they become prerequisites, because the normal path is otherwise unauthorized.** **Unchanged by the `EM-OPEN-022` ruling (2026-08-15):** that ruling settled **the identity of the opportunity after a correction**, not **the extent of the correction authority**. **Every boundary in this list remains open.**
