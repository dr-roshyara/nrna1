...
|---|---|---|
| **`EM-VOC-006`** | **A tolerance period is a governed span of time, fixed in advance and attached to a scheduled election phase, during which an authorized request to progress that phase may still be considered.** It is a property of the schedule — **not a decision, not a permission, not a progression** — and **it never becomes a published time**: a 10:00 start with 30 minutes' tolerance remains a 10:00 start. | 🟡 **PROPOSED** |
| **`EM-VOT-006`** | **A tolerance period never authorizes progression.** It establishes only that a request made within it may still be considered. **Progression still requires the authorized request and the mandatory Election conditions, exactly as at the scheduled time.** | 🟡 **PROPOSED** |
| **`EM-GOV-006`** | **A tolerance exists only where an Election Rule deliberately establishes it. No technical behaviour constitutes a tolerance** — not a cron/scheduler grace period, hard-coded allowance, retry window, delayed job, clock-drift allowance, interface grace, automatic transition grace, or any implicit *"a few minutes late is fine"*. | 🟡 **PROPOSED** |

**DERIVED and binding NOW, independent of whether the above are adopted** *(consequences of `EM-VOT-004`)*: **automatic progression during a tolerance period is EXCLUDED** — a clock-driven mechanism may not exercise the Chief's authority, and Governance found no genuine conflict justifying a return of that model to the PO. **And: until a tolerance rule is adopted, the correct number of tolerances in this system is ZERO** — a later adoption does not retrospectively authorize an earlier behaviour. *(Governance has inspected no code and asserts nothing about current system behaviour; that is a verification question.)*
...
| **EM-OPEN-023** | 🔴 **What constitutes PUBLICATION of a voting schedule?** — **before the PO/ARB since 2026-08-15**; **Governance declines to steer it** (report: `…-governance-decision-report-publication-time-boundaries.md` §1). **Four parts that must travel together:** **(a)** the publication meaning itself — *distinct business act* · *defined milestone* · *no separate act, and then what makes a schedule authoritative* · **(b)** may the governing timezone change **after** publication? · **(c)** are *created* and *valid* distinct business moments for an opportunity? · **(d)** is preparation itself a material event for the protocol? ⚠️ **Constraints binding ANY answer** *(derived, pre-empting no option)*: **C-1** the moment must be recordable as a material event (`EM-GOV-005`) · **C-2** it must be determinate and non-retroactive · **C-3** it must create **no** significance threshold or "this edit doesn't count" surface — expressly rejected by the `EM-OPEN-022` ruling. ⛔ **Architecture must not choose a publication moment.** **Settled meanwhile:** a pre-publication edit is not a correction (`EM-VOC-005` D-1). *Precedent available, not authority: the `PBDIGIT-60` results ruling — publication is an immutable fact, visibility a separate control.* | voting opportunity identity |
...
| **EM-OPEN-026** | 🔴 **THE TOLERANCE CONCEPT — five parts, one decision.** **(a) 🚧 GATE: is a tolerance a GRANT of extra time or a BOUND on an otherwise-open period?** The commission carries **both** readings *(§3 "creates an **additional** governed period" vs §13 "**Permitted Decision Period**")* — **opposite rules with the same name.** With a 10:00–12:00 window and 30 min tolerance, *Grant* adds nothing at the start while *Bound* **removes 90 minutes** of otherwise-available decision time. **Sub-question: what IS the decision period when no tolerance is defined?** — no adopted rule says, and tolerance cannot be defined against an undefined baseline. · **(b)** does tolerance exist at all *(not adopted merely because proposed)* · **(c)** who **approves** a tolerance, and is there a governed maximum? *(the PO direction names who supplies the value, not who approves it — an unbounded self-set tolerance would make the published window decorative)* · **(d)** when does it become binding? **— depends on `EM-OPEN-023`** · **(e)** is it part of the **published** schedule or an internal parameter? *(Governance PROPOSED: published — a rule that decides whether an election may start should not be invisible; counter-argument recorded.)* | election schedule tolerance |
