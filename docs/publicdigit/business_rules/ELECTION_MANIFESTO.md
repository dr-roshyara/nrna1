...
| **EM-OPEN-022** | **Does a schedule correction create a NEW voting opportunity, or modify the EXISTING one?** `EM-GOV-004` permits the Chief to correct a schedule; `EM-VOT-005` requires a **new** authorization for a **new** opportunity — **the two meet at a point neither answers.** ⚠️ **BINDING CONSTRAINT UNTIL DECIDED (PO, 2026-08-15):** **no implementation may allow a previous authorization, refusal, or eligibility decision to become valid for a changed schedule unless Governance explicitly establishes that rule.** The conservative reading therefore governs by default: **a changed schedule carries nothing forward.** ② **Architecture and Implementation must not infer the answer** — the question is Governance's. ③ Governance records that the adopted anti-circumvention intent points toward *"new opportunity, re-decide"*, but **does not decide it here.** | start-of-voting authority × schedule correction |
