...
| **EM-OPEN-047** *(narrowed by the split, 2026-08-15)* | 🔴 **What does reaching the recovery deadline MEAN?** *(A · automatic cancellation · B · escalate to organisation authority · C · permit an authorized extension · D · another rule.)* ✅ **Governance records that the split RESOLVED the concern it had raised:** expiry is now **a recorded EVENT, not a termination**, so the clock **no longer terminates an election by itself** — **fully consistent with `EM-GOV-011`'s *"time triggers evaluation and decides nothing"*, and with `EM-OPEN-031`(b): an unrequested, clock-driven evaluation yields a FINDING, never a decision.** ⚠️ **If option C is chosen:** who authorizes an extension, and **can repeated extensions keep an election halted indefinitely?** — **decide with `EM-OPEN-046`.** | election recovery |
