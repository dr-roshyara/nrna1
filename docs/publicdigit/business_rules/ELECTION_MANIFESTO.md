...
| **EM-OPEN-042** *(re-framed twice, 2026-08-15)* | 🔴 **What should the ELECTION become when a REACHED phase cannot complete because a mandatory condition is not satisfied?** *(Plain-language form: administration scheduled 1–5 Sept · evaluated on the 5th · zero voters · cannot complete · failure recorded — **what is the election now?**)* ⚠️ **Governance's earlier draft asked *"does it enter Suspension?"* — that presupposed its own answer; suspension is ONE candidate, not the frame.** **Nine parts: (a)** await authorized corrective action · **(b)** reschedule the failed phase · **(c)** restart from that phase · **(d)** restart from an earlier phase · **(e)** require a new application · **(f)** cancel/abandon · **(g)** **who decides the recovery path — the Chief, or do the Election Rules determine what recovery is permitted?** · **(h)** what is recorded · **(i)** **what becomes of the already-created downstream opportunities?** **Candidate philosophies, none endorsed** — incl. **a rule that depends on the REASON for failure**. **Invariant status stated honestly:** *no phase outcome without reachability* is **already secured** (`EM-GOV-011`); *no election-level outcome inferred from one phase failure* is **true only by ABSENCE — absence is not a rule, and one line would adopt it.** ⚠️ **IF suspension is chosen, the naming blocker applies first: *suspension* already means MEMBER suspension** (`EM-GOV-002`/`003`, `EM-EO-004`, `EM-FM-003`/`004`, opens `002`/`003`/`006`/`007`/`009`) — **under a shared word, *"the Chief may independently suspend"* would read as authorizing suspension of the ELECTION.** **Derived already, whatever path is chosen: recovery can never bypass an Election Rule** (`EM-GOV-008` + `EM-VOC-005`/`EM-VOT-005`) **and every event is recorded** (`EM-GOV-005` + negative half). **`EM-OPEN-035` and (g) are one authority model. `EM-OPEN-009` travels with this.** | election lifecycle semantics |
