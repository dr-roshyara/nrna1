...
| **`EM-GOV-014`** | 🟡 **PENDING — one line adopts it.** **Recovery deadline + responsibility.** *"When election progression is halted, the election enters a recovery period and the Election Chief is responsible for initiating a governed recovery action within 30 calendar days of the recorded halt. The organisation may replace the Election Chief during or after that period according to its governance rules; a replacement Chief inherits responsibility for deciding the permitted recovery path but cannot bypass any Election Rule or reuse invalidated decisions. If no valid recovery action is initiated within the period, the election is cancelled, and the cancellation and its reason are permanently recorded. Failure of a phase does not automatically cancel the election — cancellation occurs only because the recovery period expired without an authorized recovery action. After cancellation the old election cannot be continued; a new governed election process is required."* **Invariant stated with it: replacing the Chief does NOT reset history — the protocol remains the source of truth.** | 🟡 **PENDING PO ADOPTION** *(offered as *"I would formulate"* / *"my recommendation"* — a draft, not a performative act)* |
...
| **EM-OPEN-047** | 🔴 **After the recovery period expires, is cancellation AUTOMATIC, or does the organisation have one final decision point** *(appoint a new Chief · approve an exceptional recovery)*? **The PO's own narrowing, registered as the open question.** ⚠️ **Governance notes what hangs on it: under the automatic reading this would be the FIRST and ONLY place in the whole model where THE CLOCK ALONE TERMINATES AN ELECTION.** It contradicts no adopted rule — `EM-VOT-004` forbids the clock exercising the Chief's authority to **progress**, and cancellation is not progression — **but it is the philosophical inverse of everything else adopted, and must be chosen knowingly rather than inherited from a deadline.** | election recovery |
| **EM-OPEN-048** | ⚠️ 🔴 **Naming and parameter questions raised by the recovery period.** **(a) FIFTH collision risk — a *recovery period* is a governed time-window like the *post-schedule decision period*, but anchored to a HALT EVENT rather than a scheduled phase.** **They must be named apart.** *(Note: the recovery period is unambiguously a **BOUND** — a deadline with a consequence — whereas whether the post-schedule period is a grant or a bound is still `EM-OPEN-026`(a). Different animals; identical-looking names would hide that.)* · **(b) SIXTH overload risk — *cancelled*.** `EM-VOC-004` already uses it for **an opportunity outcome**; this rule uses it for **an election-level terminal state.** Two levels, one word. · **(c) Where does 30 days come from?** **Nothing in the record derives it** — a legitimate business choice, but it must be recorded as **chosen, not derived.** · **(d) Uniform or per-election?** ⚠️ **Under `EM-GOV-009` the complete schedule is fixed at APPLICATION; the window allowance was also to be given per window at application. **Why is recovery a single global figure?** A consistency question, not an objection. | vocabulary · parameters |
