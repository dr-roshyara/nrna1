...
| **EM-OPEN-045** | 🔴 **Does a phase that was REACHED and failed have an outcome of its own — and is it "failed"?** ⚠️ **Still unruled.** The illustrative event name *AdministrationFailed* appears in the PO's worked model, **but an event name in an example is not a ruling** and Governance does not read it as one. *(Distinct from the never-reached phases, which need nothing — `EM-GOV-011`.)* | phase outcomes |
