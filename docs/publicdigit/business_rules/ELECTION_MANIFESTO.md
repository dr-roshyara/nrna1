...
| **EM-OPEN-031** | 🔴 **At the scheduled start, does anything evaluate and RECORD when no request has been made?** The PO's model shows *scheduled start reached → **protocol records evaluation***. **If nothing is triggered, nothing is recorded** — so recording an evaluation at the scheduled time implies a **clock-driven act that writes to the protocol**. **This does not violate `EM-VOT-004`** *(it progresses nothing)*, **but it must be deliberate, not incidental**, and it **contradicts Governance's own PROPOSED position** that *"scheduled time reached"* is not material in itself (tolerance report §11). ⚠️ **Governance flags its own proposal as the weaker candidate here:** an evaluation that was **performed** is an act and is plausibly material, even when the answer is *cannot proceed*; a clock merely **passing** a time is not. **The PO's principle — *"if 10:00 arrives and a condition fails, the system records that fact"* — points to recording the evaluation.** Not decided. | election protocol × `EM-GOV-005` |
...
| **EM-OPEN-037** | 🔴 **(a) ⚠️ Does "opportunity" generalise beyond voting?** `EM-VOC-004` is **voting-scoped by its adopted text**. Speaking of an administration/candidacy/counting/result-publication opportunity extends it — and since an opportunity is a context awaiting an authorized progression request, **that implicitly answers `G-PROG-1` and `EM-OPEN-030` for every phase, making ALL phases request-progressed.** May be the intent; **must not arrive by inheritance.** · **(b) 🔴 REFRAMED (PO, 2026-08-15) — the earlier form was too simplistic.** *When the complete schedule is officially published, does publication establish the **planned** phase opportunities while the actual availability and progression of each phase remain subject to the preceding phase completing its mandatory conditions?* **A · planned opportunities are created at publication and remain DORMANT until their predecessor legitimately completes** — the published plan is a fact, and the protocol can later say *"Voting was scheduled for 11 September, but Candidacy did not legitimately complete, therefore Voting never became eligible."* **B · a phase becomes an opportunity only when its predecessor successfully completes.** **PO leaning recorded, NOT a decision: A** *(richer than pretending the phase never existed)*. ⚠️ **Governance gap under A, which must be closed if A is chosen: `EM-VOC-004`'s five outcomes cannot name what happens to a planned opportunity that NEVER BECAME AVAILABLE.** *Expired unused* implies it was available and unused; *cancelled* implies an act; *superseded* implies a later schedule. **A sixth outcome — or an explicit ruling that one of the five covers it — is required.** | opportunity scope × phase dependency |
| **EM-OPEN-038** | 🔴 **The phase dependency chain — adopt it explicitly; it must not arrive by description.** The model: administration must legitimately complete *(electorate established, posts established)* → candidacy *(nominated, evaluated, closed)* → voting *(approved candidates, admitted voters, conditions, Chief's request)* → counting *(voting legitimately closed)* → result publication. **No adopted rule states this chain**, and it is a business prerequisite rule, not merely a schedule-ordering constraint *(it sharpens `EM-OPEN-033`(c), which asked only about ordering)*. **DERIVED already and not in dispute:** *"the clock says 6 September, therefore start candidacy"* is excluded — `EM-VOT-004`'s principle, whose generalisation to other phases is `EM-OPEN-037`(a). ⚠️ **CASCADE QUESTION, surfaced here:** under plan-first, if an early phase cannot complete in its window, **every later window may need correcting too — so ONE failure may produce SEVERAL corrections, each superseding its own opportunity and each requiring a new authorization.** **Does correcting one window require, permit, or forbid correcting the downstream windows in the same governed act?** No rule says. | phase progression |
| **EM-VOC-CAUTION** | ⚠️ **VOCABULARY CAUTION (Governance, 2026-08-15) — recorded, not adopted.** The distinction offered — **scheduled ≠ eligible ≠ actionable ≠ active** — is useful, **but "eligible" already has an adopted meaning**: `EM-VOT-004` says reaching the scheduled time *"makes the election **eligible for the Chief Election Officer's consideration**"* — i.e. **eligible is TIME-BASED ONLY.** In the new four-way distinction, *eligible* appears to mean *time reached **and** prerequisites satisfied*. **Two meanings for one word in the same domain.** ⛔ **Before adopting the four-way distinction, either keep `EM-VOT-004`'s meaning and name the prerequisite-satisfied state differently, or amend `EM-VOT-004` deliberately. "Active" also sits close to lifecycle vocabulary and should be business-named.** | vocabulary integrity |
