...
| **A-8** *(recorded 2026-08-17 at architect review — R-A of the review)* | Application orchestration decomposition: whether consequence sequencing (§4) later moves from handlers into an explicit application process manager, to prevent handlers becoming mini workflow engines | not now — recorded for observation; revisit if Increment-2 RED shows a handler exceeding one primary act + its design-flow consequences |
| **A-9** *(surfaced by the 2026-08-17 boundary review)* | Halt-onset consequence policy: the sequencing "decided failure ⇒ halted at the gate ⇒ halted-recovery period starts" (F-2) has no domain policy analogous to P-4 `InoperativeOnset`; Increment 2 realizes it as handler orchestration traceable to F-2 (domain frozen). Whether a domain-side onset policy should exist is a future authorized **domain** slice's decision, never an Increment-2 act | future domain slice, own authorization; Increment-2 RED pins the F-2 sequence meanwhile |
