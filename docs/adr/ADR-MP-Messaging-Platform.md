...
**Constitutional Conversation (architectural term, ARB 2026-07-10):** the complete causal chain of events originating from ONE constitutional decision process — e.g. `Challenge → Determination → Correction → Resolution`, or later `Membership → Election → Vote → Result → Audit`. A Constitutional Conversation is identified by a single CorrelationId; every event in it shares that CorrelationId; every event except the first records its immediate causal predecessor as the CausationId.
**Architecture Fitness Rule — CorrelationId minting:** exactly ONE producer may mint a CorrelationId per Constitutional Conversation; all subsequent producers propagate it UNCHANGED; no producer within a conversation may mint a second one. Enforced by: architecture (`EventProvenance::start()` used only by chain-starting producers) · design (`fromConsumed()` propagates, never mints) · test (IT-1 asserts one CorrelationId across all rows). Executable verification (a dedicated fitness test) is **deferred to PB-007 or the first architecture-quality milestone after it** — a concrete roadmap slot, not an indefinite candidate.
**Acyclicity invariant (ARB 2026-07-10 — recorded for future evolution, no implementation today):** a Constitutional Conversation must form a **directed acyclic graph** — causation links only ever point to earlier events; no event may directly or transitively cause itself. This becomes load-bearing when replay, projections, and distributed workflows arrive.
**EventProvenance belongs ONLY to Integration Events — never Domain Events:** Domain Events carry ZERO provenance (they are business facts); Integration Events carry `EventProvenance` (the published message with lineage). **Invariant: no Domain Event class may contain CorrelationId or CausationId properties** — provenance lives exclusively in the messaging layer. This preserves bounded-context purity and lets any future transport reuse `EventProvenance` unchanged.
