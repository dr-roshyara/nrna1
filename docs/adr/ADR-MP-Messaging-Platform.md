...
**Architecture Fitness Rule — CorrelationId minting:** exactly ONE producer may mint a CorrelationId per Constitutional Conversation; all subsequent producers propagate it UNCHANGED; no producer within a conversation may mint a second one. Enforced by: architecture (`EventProvenance::start()` used only by chain-starting producers) · design (`fromConsumed()` propagates, never mints) · test (IT-1 asserts one CorrelationId across all rows). Candidate for a dedicated fitness test at the retrospective.
