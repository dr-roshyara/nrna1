...
| **ADR-T17** | **LegitimacyDecision placement — DEFERRED (Proposed)** | legitimacy is currently a command input; the `LegitimacyDecision` policy (50-06) must run in the Determination aggregate or an Adjudication domain service, **never** the application service | resolve at the start of Push B (Election reaction / legitimacy reasoning) | R1.1 review Q3 · 50-06 |
