...
- **Naming candidate added to the R-35 bundle (ARB, 2026-07-10, Class A):** **"Evidence-Driven Engineering Platform"** — ARB: "AI Engineering Platform" is no longer the right description; AI is one execution adapter; the platform remains valuable even if the LLM landscape changes dramatically. Weigh against R-35's adopted "PublicDigit Engineering Platform" at the supersession (the two answer different questions: whose platform vs. what kind); decide once, at the retrospective.
- **ARB phased outlook on record (2026-07-10, Class A — judgment, not measurements):** P1 C3→PB-004→retrospective ("its job is not to invent; expect many rejections — that is success") · P2 executable qualification (metrics pipeline) · P3 AI reads evidence · P4 generated dashboard · P5 second adopter · P6 only then revisit research hypotheses. Explicit not-do list re-confirmed: no new ADRs, folder reorgs, domain redesigns, renames, custom metrics engines, capability hierarchies.
...
- **ARB observation — code metrics as evidence sources for fitness functions (2026-07-10, Class A; explicitly AFTER C3, not before; ARB disposition: APPROVED for retrospective consideration, 10/10; SINGLE CANONICAL ENTRY — evolves in place, revision history at the end):** LCOM4/CBO/RFC/DIT become **evidence sources for fitness functions — never fitness functions themselves** (`LCOM4 = 5` is a measurement; the fitness function is the RULE that consumes measurements). **Lifecycle vocabulary (ARB, formalized):** **Measurement** (tool output, `LCOM4 = 5`) → **Evidence** (machine-readable artifact, PHPMetrics JSON) → **Fitness Function** (rule consuming evidence, "no aggregate exceeds the LCOM threshold") → **Qualification Result** (PASS/FAIL — a result is NOT yet a decision: Deptrac 0 violations → PASS is merely a result) → **Architectural Interpretation** (the AI's role: *why did it pass? what changed? what trend? what should improve?*) → **Architectural Decision** (the ARB's role: accept · reject · record · promote). Authority chain: *deterministic tools → qualification result → AI architectural interpretation → **human decision*** — the AI is never the final authority. Thresholds, if any, are **established only after implementation evidence has accumulated** (measured → observed → trend → threshold-maybe; the Infection philosophy — metrics rarely start as PASS/FAIL), computed by a deterministic analyzer (recommended: phpmetrics; NOT a custom PHP-Parser engine, months of work) — never calculated by the AI. **Category distinction preserved (ARB):** *structural metrics* (LCOM4/CBO/RFC/DIT — they DESCRIBE) ≠ *architectural fitness rules* (Deptrac, hexagonal rules, CorrelationId guard, messaging ownership — they PRESCRIBE); never conflate. Division of labor fixed: *deterministic tools produce measurements; the AI reads the artifacts and writes the architectural interpretation* (exactly the PHPUnit relationship — Claude reads test output, never executes tests in its head). ARB sequence: (1) C3 gate runner → (2) PHPStan/Deptrac/PHPUnit/PHPMetrics emit machine-readable artifacts (target: reserved `engineering/verification/` namespaces) → (3) qualification consumes artifacts → PASS/FAIL evidence → (4) AI generates interpretation/recommendations from evidence. **Partial evidence already exists on the product track:** PB-007's `composer merge-gate` (PHPStan+Deptrac+PHPUnit+Infection, captured MSI/MCC baseline, commit c3104b644) is steps 1–2 in embryo — an incremental extension of an existing qualification pipeline, NOT a new subsystem. Feeds the dashboard observation (everything derived). **Formal ARB disposition (2026-07-10, verbatim):** *"Approved. This observation correctly follows the Engineering Platform promotion model. No implementation is authorized at this time. The proposal is supported by existing implementation evidence from PB-007 and therefore satisfies the burden-of-proof requirement for retrospective consideration. Deterministic analysis tools remain the exclusive source of structural measurements; the AI consumes those artifacts to produce architectural interpretation and recommendations. Any future thresholds or fitness rules derived from these metrics require separate retrospective approval after sufficient longitudinal evidence has been collected."* `engineering/` now separates *ownership* well but not yet *lifecycle*. Candidate lifecycle categories (NOT folders; folder-hood decided by implementation evidence): Engineering Knowledge (patterns, research) · Engineering Decisions (ADR, rulings) · Engineering Evidence (qualification, reports) · Engineering Runtime (registry, bindings) · Engineering Adoption (project-specific evidence). Decide with the coupled tree decision at the retrospective. **Revision history (rules live once — this entry is the only home):** r1 "fitness functions" → r2 "engineering fitness indicators, thresholds after longitudinal evidence" → r3 "evidence sources for fitness functions" + four-level vocabulary → r4 fifth level **Architectural Interpretation** added + authority chain ends at **human decision** (AI never final authority) → r5 **Qualification split into Qualification Result ≠ Architectural Decision** (a PASS is a result; only the ARB's accept/reject/record/promote is a decision), all ARB 2026-07-10. **Post-retrospective normalization recorded (ARB):** this entry's inline rN chain is transitional; after the retrospective, normalize canonical observations into `Observation · Decision · Rationale · Revision History · References` (ER-05 preserved, readability restored).
- **ARB observation — the observation inbox itself (2026-07-10, Class A; EVIDENCE ONLY — no restructuring authorized before the retrospective):** this inbox has become sufficiently large that it is itself showing candidate bounded contexts (dashboard · aggregate folders · metrics · capability · lifecycle · platform-vs-adoption — a research notebook + retrospective inbox + ADR queue interwoven in one document). This is the same pressure PublicDigit resolved by separating Architecture/ADR/guides/sessions/plans — recorded here as **implementation evidence for the retrospective**, exactly per the freeze rule (Freeze → Evidence → Retrospective → Promotion). **ARB standing instruction:** before adding a new observation here, ask whether it belongs to an emerging engineering bounded context; keep recording during the freeze; if the retrospective confirms coherent domains (Engineering Knowledge · Evidence · Runtime · Adoption), promote them **together in a single architectural decision**, never through incremental reorganizations. **Candidate invariant recorded for the retrospective (ARB — NOT adopted, R-27 freeze):** *"Every engineering observation must identify its lifecycle stage before it identifies its content"* — explicit states (research hypothesis · observed implementation evidence · candidate practice · candidate standard · approved standard · deprecated standard) instead of stage-by-prose; today this inbox mixes all of them, differentiated only by wording.
- **ARB observation @ F-7D-2 experiment execution (2026-07-10; EVALUATE at retrospective, NOT promote):** the precondition check used before resuming the evidence-validation run (measured source unchanged · measured tests unchanged · coverage artifacts reused · no concurrent test activity) is a recurring pattern — evaluate whether it deserves to become a standard **"Evidence Preconditions" checklist** for long-running engineering experiments. Practice → Evidence → Retrospective → Promotion; evidence attached: the F-7D-2 execution contract (PB-007 IDD §2e).
