...
- **ARB observation — Architecture Qualification Dashboard (2026-07-10, Class A; explicitly AFTER PB-004, "belongs after C3, not before"):** a **generated** operational view over the evidence — never another document/ADR (governance ✓/✗ per area · gate verdicts · cold-boot PASS/FAIL · maturity level). Honesty constraint carries over: every value **derived from recorded evidence** (registry parse, gate outputs, qualification records) — the dashboard renders what the instruments measured, it never asserts (AIP-10/R-26); a value with no evidence source is not displayed. Natural consumer of the reserved `verification/evidence/` namespace; queue beside EPC-014 (Engineering Confidence) at the retrospective.
