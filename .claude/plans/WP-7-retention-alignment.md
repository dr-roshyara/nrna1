...
**Status:** ✅ **SLICE 7B PLANNING APPROVED WITH REQUIRED CORRECTIONS (R-56)** · ✅ **P7B-2 SETTLED** (fallback => application service) · ⚠️ **P7B-3 NEW (ARB's): which slice hosts K9** — allocating the fallback to the application service may move K9 **out of 7B**, since §4 calls that service *the guard* (7C's subject) while 7B's tests still name it. ⛔ **7B EXECUTION NOT AUTHORIZED (queue 13).** **THREE governance items before RED: P7B-1 · P7B-3 · execution authorization. Engineering has none.** EPW anchor open, non-blocking. WP-6 CLOSED (R-55); merge gate PASSES. **7C unauthorized · F-WP6R-1 excluded.**
...
| ⚠️ **P7B-1 — STALE MECHANISM, flagged 2026-08-01, NOT corrected here** | This row still reads *"consuming `AdjudicationDurations` for MAD"* and *"MAD comes from the **existing** port"*. **R-44 superseded that mechanism**: Election declares its **own** `EvidencePreservationDurations` port, and importing Adjudication's is the **TP-1 violation R-44 rejected**. **The INVARIANT — MAD keeps exactly one home — is untouched; only the mechanism sentence is stale.** Correcting an approved plan is a **governance act**, so it is **referred to the ARB / Decision Authority, not edited here.** Same stale-reference class corrected for 7A in the alignment commission. |
...
| ⚠️ **P7B-3 — NEW, ARB's: which SLICE hosts K9** | P7B-2's allocation produced it. **§4 defines exactly ONE application service and calls it *the guard* — 7C's subject — yet 7B's test list and acceptance criterion both name *anchor-absent => open*.** So K9 may fall **outside 7B**. Readings: **(a)** service is 7C's => K9 moves to 7C and 7B's tests+acceptance need correcting · **(b)** 7B delivers the VO **and** its building service => K9 stays. **A slice-boundary question in an approved plan — the ARB's, same class as P7B-1. Settle before RED**, or 7B's RED targets a keystone the slice cannot satisfy. |
