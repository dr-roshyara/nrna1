...
- **Rule parsimony (ARB 2026-07-10 — no more freezes/rules):** the constraint set is complete (R-27 claims · R-37 structure · EEP execution · score-stop recording · inbox freeze backlog). On any recurring problem, FIRST ask *"does an existing rule already cover this?"* — prefer interpreting existing rules over creating new ones; a new rule requires a genuine "no".
...
- **Repository documents RECORD governance; they do not CREATE it (ARB standing rule, 2026-07-11).** Only explicit ARB decisions create governance. Words like *continue*, *looks good*, *go ahead* are workflow permission — they must NEVER be interpreted as Approved/Promoted/Retired/Closed. When a governance decision is needed, STOP and ask per item (Approve / Reject / Defer). Violation precedent: a "continue" was once read as confirmation and had to be retracted (EPIC-001 closure, 2026-07-11).
- **Retrospectives: RECOMMEND governance decisions, never DECLARE them (ARB standing instruction, 2026-07-11).** The AI is researcher/recorder/analyst; the ARB is the authority. Language: "Recommended for Promotion/Retirement" + evidence + "ARB decision: PENDING" — a decision becomes PROMOTED/RETIRED only when the ARB confirms. Authority chain: implementation → evidence → AI analysis → ARB decision → standard.
