...
- **→ STOP. Next (authorized, ONE slice) — Step 4A.2: DESIGN the Election existence Anti-Corruption Layer** over the legacy `elections` table (read-only existence/identity behind a port; greenfield stays autonomous; Strangler). **No persistence code** — design/decision only. Then 4A.3 persistence · 4B messaging (outbox adapter · inbox wiring · hydrator) · 4C qualification (extend fitness scan · regression · docs · Completion Review). Messaging unaffected (consume-only, ARR PASS).
