...
- **✔ Step 3 (PB-004) — RED REFINED (ARB round 2); AWAITING REVIEW before GREEN.** Failing tests only (11 failed; no production, no Election context). `tests/Unit/Contexts/Election/`:
  ...
  - `DeterminationIssuedReactionHandlerTest`: Shared InboxHandler; consumes schema v2 + reconstructs local ElectionId (ADR-T16); **schema v1 (no contestedOutcome) → PermanentInboxFailure** (dead-letter, explicit back-compat); **unknown election → DERIVED from the authority-issued determination, not rejected** (constitutional decision — flagged for ARB).
  - Behavior-first (ER-07). ARB-6 refinements folded (aggregate verb · policy name · forward-only · event boundary · v1 behavior · missing-election behavior).
