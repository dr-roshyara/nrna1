...
- **→ STOP. Next slice (authorized, one slice then stop) — Election Infrastructure, TDD RED-first:** (0) clock-based `appliedAt` correction · (1) tenant-scoped Eloquent `ElectionRepository` · (2) `ReactionEventOutbox` outbox adapter (same txn, ADR-T1) · (3) `elections` migration · (4) inbox-registry wiring · (5) `ElectionCorrectionApplied` hydrator · (6) extend `GreenfieldCoreArchitectureTest` to Election (complete hexagonal context). Messaging unaffected (consume-only, ARR PASS).
