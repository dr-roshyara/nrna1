...
- **→ STOP for EP-02 Completion Review.** Next slice (on approval): Election **Infrastructure** — Eloquent `ElectionRepository` (tenant-scoped) + `ReactionEventOutbox` outbox adapter (ADR-T1) + migration + inbox-registry wiring + `ElectionCorrectionApplied` hydrator; then extend the fitness scan to Election. Messaging unaffected (consume-only, ARR PASS).
