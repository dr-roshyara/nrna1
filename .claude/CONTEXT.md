...
- **✔ Step 3 (PB-004) — RED written; AWAITING ARB REVIEW before GREEN.** Failing tests only (no production, no Election context yet): `tests/Unit/Contexts/Election/` — CorrectionTypeDecision (Dismissed→none D-02, Upheld→ContainedOnly ADR-T8) · Election aggregate reaction (event emitted · Dismissed→no event · aggregate-level idempotency · anonymity of the event) · handler consuming `DeterminationIssued` schema v2 + reconstructing its own local model (ADR-T16). RED = 8 failed (context absent). Behavior-first (ER-07). ER-07 added to Process v1.1 draft.
- **→ STOP. Do NOT write GREEN/production** until ARB reviews the RED. Then GREEN = create `app/Contexts/Election/{Domain,Application}` minimal reaction slice. Messaging unaffected (consume-only, ARR PASS).
